...
- [ ] `POST /v1/auth/login` exchanges an app token/founder key for a central session token.
- [ ] `GET /v1/auth/me` validates central sessions and returns app/org context.
- [ ] `POST /v1/auth/logout` revokes central sessions.
...
- [ ] Public lanes accept central session bearer tokens without requiring raw app tokens in browser clients.

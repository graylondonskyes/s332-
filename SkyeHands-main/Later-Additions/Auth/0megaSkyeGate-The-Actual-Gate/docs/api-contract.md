...
### `POST /v1/auth/login`
Accepts an existing app token or founder gateway key and returns a central `session_token`.
The session token is the preferred browser/runtime bearer token for downstream Sky0s apps.

### `POST /v1/auth/logout`
Revokes the presented central session token.

### `GET /v1/auth/me`
Validates the presented central session token and returns the caller's app/org context.

### `GET /v1/models`
Requires app bearer token or central session bearer token.

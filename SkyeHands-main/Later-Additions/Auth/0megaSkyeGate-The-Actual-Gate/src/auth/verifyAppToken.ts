...
import { findAppAuthContextByAppId, findAppTokenByHash } from '../db/queries'
...
export async function verifyAppToken(request: Request, env: Env): Promise<AuthContext> {
  ...
  if (!auth) {
    ...
    try {
      ...
      if (session.authMode === 'founder-gateway') {
        return createFounderGatewayAuth()
      }

      const sessionAuth = await findAppAuthContextByAppId(requireDb(env), session.appId)
      if (!sessionAuth) {
        throw new KaixuError(401, 'KAIXU_UNAUTHORIZED', 'This Kaixu route is not authorized for the current caller.', {
          adminDetail: `Session app is not backed by an enabled app token: ${session.appId}`,
        })
      }
      ...
      if (sessionError instanceof KaixuError) {
        throw sessionError
      }
      throw new KaixuError(401, 'KAIXU_UNAUTHORIZED', 'This Kaixu route is not authorized for the current caller.', { adminDetail: 'App token or session token not recognized.' })
    }
  }

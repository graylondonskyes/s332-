...
import { stripDataUrlPrefix, base64ToBytes, bytesToArrayBuffer } from '../utils/base64'
...
export async function callOpenAIVideoCreate(model: string, request: SkyVideoRequest, env: Env, alias: KaixuEngineAlias): Promise<NormalizedVideoCreateResult> {
  ...
  if (request.reference_image_base64) {
    ...
    form.set('image', new File([bytesToArrayBuffer(base64ToBytes(base64))], 'reference.png', { type: request.reference_image_mime_type || mimeType || 'image/png' }))

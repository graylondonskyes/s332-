...
import { stripDataUrlPrefix, toDataUrl, base64ToBytes, bytesToArrayBuffer } from '../utils/base64'
...
export async function callOpenAIImages(model: string, request: SkyImageRequest, env: Env, alias: KaixuEngineAlias): Promise<NormalizedImageResult> {
  ...
  if (request.input_image_base64) {
    ...
    form.set('image', new File([bytesToArrayBuffer(base64ToBytes(base64))], request.input_image_mime_type ? `input.${request.input_image_mime_type.split('/').pop()}` : 'input.png', { type: request.input_image_mime_type || mimeType || 'image/png' }))

...
test("AI agent routes and schema exist", () => {
  const s = read("schema.sql");
  const a = read("netlify/functions/api.mjs");
  assert.match(s, /CREATE TABLE IF NOT EXISTS org_ai_settings/i);
  assert.match(s, /ALTER TABLE projects ADD COLUMN IF NOT EXISTS settings_json/i);
  assert.match(a, /\/ai\/agent/);
  assert.match(a, /\/ai\/settings/);
  assert.match(a, /org_ai_settings/);
  assert.match(a, /tool_calls/);
});

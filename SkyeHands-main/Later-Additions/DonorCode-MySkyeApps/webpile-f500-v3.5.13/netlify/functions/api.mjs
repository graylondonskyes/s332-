...
function normalizeAiModel(model) {
  const raw = String(model || OPENAI_DEFAULT_MODEL || 'gpt-4o-mini').trim();
  return (raw || String(OPENAI_DEFAULT_MODEL || 'gpt-4o-mini')).slice(0, 120);
}

function normalizeProjectSettings(input = {}) {
  const obj = (input && typeof input === 'object') ? input : {};
  const device = ['responsive', '1440', '1280', '1024', '768', '430', '390'].includes(String(obj.device || 'responsive')) ? String(obj.device || 'responsive') : 'responsive';
  const scale = ['1', '0.9', '0.8', '0.7'].includes(String(obj.scale || '1')) ? String(obj.scale || '1') : '1';
  return {
    auto: obj.auto !== false,
    fullMode: !!obj.fullMode,
    isolated: obj.isolated !== false,
    instrument: obj.instrument !== false,
    network: String(obj.network || 'block') === 'allow' ? 'allow' : 'block',
    debounce: clampInt(obj.debounce ?? 350, 0, 5000, 350),
    device,
    scale,
  };
}

async function optionalSessionContext(event) {
...
async function getAiSettingsSummary(orgId) {
  const rows = await sql`
    SELECT provider, default_model, autonomy_steps, updated_at, (api_key_enc IS NOT NULL AND api_key_enc <> '') AS has_api_key
  ...
  if (!rows || rows.length === 0) {
    return {
      provider: 'openai',
      model: normalizeAiModel(OPENAI_DEFAULT_MODEL),
      autonomySteps: 12,
      hasApiKey: false,
      updatedAt: null,
    };
  ...
  return {
    ...
    model: normalizeAiModel(r.default_model || OPENAI_DEFAULT_MODEL),
    autonomySteps: clampInt(r.autonomy_steps ?? 12, 1, 30, 12),
    hasApiKey: !!r.has_api_key,
    updatedAt: r.updated_at || null,
  };
}

async function resolveAiApiKey(body, ctx) {
  const explicit = String(body?.apiKey || '').trim();
  if (explicit) return explicit;
  if (ctx?.orgId) {
    const rows = await sql`SELECT api_key_enc FROM org_ai_settings WHERE org_id = ${ctx.orgId} LIMIT 1`;
    const enc = rows?.[0]?.api_key_enc;
    if (enc) return decryptSecret(enc);
  }
  if (process.env.OPENAI_API_KEY) return String(process.env.OPENAI_API_KEY);
  return '';
}
...
const AI_AGENT_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'write_file',
      description: 'Create or overwrite a file. Content must be the COMPLETE file content.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Search all files for a string and return matching lines.',
      parameters: {
        type: 'object',
        properties: {
...
function executeAiToolCall(toolName, args, workspace) {
  const files = workspace.files;
  switch (toolName) {
    case 'list_files': {
      const listing = Array.from(files.keys()).sort().map((path) => ({ path, bytes: String(files.get(path) || '').length }));
      return JSON.stringify({ total: listing.length, files: listing });
    }
    case 'read_file': {
      const content = files.get(args.path);
      if (content == null) return JSON.stringify({ error: `File not found: ${args.path}` });
    ...
    case 'write_file': {
      ...
      workspace.operations.push({ type: existed ? 'update' : 'create', path: args.path, content: String(args.content || '') });
      workspace.touched.add(args.path);
      return JSON.stringify({ ok: true, type: existed ? 'updated' : 'created', path: args.path });
    }
    case 'delete_file': {
    ...
    case 'search_codebase': {
      ...
      const regex = new RegExp(escapeRegex(q), caseSensitive ? 'g' : 'gi');
      const results = [];
...
async function openAiChatCompletion({ apiKey, model, messages, tools, temperature = 0.2, maxTokens = 2400 }) {
  const body = { model, messages, temperature, max_tokens: maxTokens };
  if (tools && tools.length) body.tools = tools;
  if (tools && tools.length) body.tool_choice = 'auto';

  const res = await fetch(`${String(OPENAI_BASE_URL || 'https://api.openai.com').replace(/\/+$/, '')}/v1/chat/completions`, {
    method: 'POST',
    headers: {
...
function aiSystemPrompt({ projectName = 'Untitled Project', activeFile = '', autonomySteps = 12 }) {
  return `You are WebPile Autonomous Editor, a project-aware coding agent running inside a browser IDE.\n\nYou may inspect the full workspace with tool calls. Read relevant files before editing them. When you write a file, always return the COMPLETE updated file content.\n\nRules:\n- Respect the existing architecture and naming already present in the workspace\n- Prefer surgical edits over random rewrites\n- Keep the app deployable on Netlify\n- When changing multiple files, update every required file in one run\n- Use search_codebase aggressively before changing cross-file behavior\n- Stop when the requested work is complete\n\nWorkspace:\n- Project: ${projectName}\n- Active file: ${activeFile || 'unknown'}\n- Max autonomous steps this run: ${autonomySteps}`;
}

async function runAiAgentLoop({ apiKey, model, prompt, files, projectName = 'Untitled Project', activeFile = '', autonomySteps = 12 }) {
  const filesMap = new Map();
  for (const f of (Array.isArray(files) ? files : [])) {
    if (!f || !f.path) continue;
    filesMap.set(String(f.path), String(f.content || ''));
  }
  const workspace = { files: filesMap, operations: [], touched: new Set() };
  const tree = Array.from(filesMap.keys()).sort().map((p) => `- ${p} (${String(filesMap.get(p) || '').length} bytes)`).join('\n');
  const messages = [
    { role: 'system', content: aiSystemPrompt({ projectName, activeFile, autonomySteps }) },
    { role: 'user', content: `${String(prompt || '').trim()}\n\nWorkspace file tree:\n${tree || '- (empty project)'}` }
  ];
  const usage = { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 };
  let iterations = 0;

  while (iterations < autonomySteps) {
    iterations += 1;
    const response = await openAiChatCompletion({ apiKey, model, messages, tools: AI_AGENT_TOOLS, temperature: 0.2, maxTokens: 2600 });
    const choice = response?.choices?.[0];
    if (!choice || !choice.message) throw new Error('No response from OpenAI');
    usage.prompt_tokens += Number(response?.usage?.prompt_tokens || 0);
    usage.completion_tokens += Number(response?.usage?.completion_tokens || 0);
    ...
    for (const tc of (msg.tool_calls || [])) {
      ...
      const result = executeAiToolCall(tc.function?.name, args, workspace);
      messages.push({ role: 'tool', tool_call_id: tc.id, content: result });
    }
  }

  return {
    reply: `Agent stopped after ${autonomySteps} steps. Review the proposed file operations.`,
    operations: workspace.operations,
    touched: Array.from(workspace.touched),
    usage,
    iterations,
...
function scimParseFilter(input) {
  ...
  const wholeBracket = raw.match(/^(emails|members)\[(.*)\]$/i);
  if (wholeBracket) {
    return { type: 'bracket', base: wholeBracket[1].toLowerCase(), ast: scimParseFilter(wholeBracket[2]) };
  }

  const toks = scimTokenize(raw);
  let pos = 0;
  ...
  function parseValue() {
    const p = peek();
    if (!p) throw new Error('scim_filter_expected_value');
    if (p.t === 'STR' || p.t === 'NUM' || p.t === 'BOOL') { pos++; return p.v; }
    throw new Error('scim_filter_expected_value');
  }
  ...
  function parsePrimary() {
    ...
    if (mbr) return { type: 'bracket', base: 'members', ast: scimParseFilter(mbr[1]) };
    if (em) return { type: 'bracket', base: 'emails', ast: scimParseFilter(em[1]) };

    const opTok = req('OP');
    if (opTok.v === 'pr') return { type: 'cmp', attr: id, op: 'pr', value: null };
    const value = parseValue();
    return { type: 'cmp', attr: id, op: opTok.v, value };
  }

  function parseNot() {
    if (eat('NOT')) return { type: 'not', node: parseNot() };
    return parsePrimary();
  }

  function parseAnd() {
    let node = parseNot();
    while (eat('AND')) node = { type: 'and', left: node, right: parseNot() };
    return node;
  }

  function parseOr() {
    let node = parseAnd();
    while (eat('OR')) node = { type: 'or', left: node, right: parseAnd() };
    return node;
  }

  const ast = parseOr();
  if (peek()) throw new Error('scim_filter_trailing_tokens');
  return ast;
}

function scimCompileFilter(ast, kind) {
  const params = [];
  const p = (v) => { params.push(v); return `$${params.length}`; };

  function compileText(col, op, value) {
    if (op === 'eq') return `${col} = ${p(value)}`;
    if (op === 'ne') return `${col} <> ${p(value)}`;
    if (op === 'co') return `${col} ILIKE '%' || ${p(value)} || '%'`;
    if (op === 'sw') return `${col} ILIKE ${p(value)} || '%'`;
    if (op === 'ew') return `${col} ILIKE '%' || ${p(value)}`;
    if (op === 'pr') return `${col} IS NOT NULL AND ${col} <> ''`;
    throw new Error('scim_filter_unsupported_op');
  }

  function compileTime(col, op, value) {
    if (!['eq','ne','gt','ge','lt','le'].includes(op)) throw new Error('scim_filter_unsupported_op');
    const rhs = `${p(value)}::timestamptz`;
    if (op === 'eq') return `${col} = ${rhs}`;
    if (op === 'ne') return `${col} <> ${rhs}`;
    if (op === 'gt') return `${col} > ${rhs}`;
    if (op === 'ge') return `${col} >= ${rhs}`;
    if (op === 'lt') return `${col} < ${rhs}`;
    if (op === 'le') return `${col} <= ${rhs}`;
    throw new Error('scim_filter_unsupported_op');
  }

  function compileBool(col, op, value) {
    if (op === 'eq') return `${col} = ${p(!!value)}`;
    if (op === 'ne') return `${col} <> ${p(!!value)}`;
    if (op === 'pr') return `${col} IS NOT NULL`;
    throw new Error('scim_filter_unsupported_op');
  }

  function compileCmp(attr, op, value) {
    const al = String(attr || '').toLowerCase();
    if (kind === 'users') {
      if (al === 'username' || al === 'email') return compileText('u.email', op, value);
      if (al === 'id') {
        if (!['eq','ne'].includes(op)) throw new Error('scim_filter_unsupported_op');
        return op === 'eq' ? `u.id = ${p(value)}::uuid` : `u.id <> ${p(value)}::uuid`;
      }
      if (al === 'active') return compileBool('m.active', op, value);
      if (al === 'meta.created') return compileTime('u.created_at', op, value);
      if (al === 'meta.lastmodified') return compileTime('u.updated_at', op, value);
      if (al === 'emails.value') return compileText('u.email', op, value);
    ...
    if (kind === 'groups') {
      ...
      if (al === 'id') {
        if (!['eq','ne'].includes(op)) throw new Error('scim_filter_unsupported_op');
        return op === 'eq' ? `g.id = ${p(value)}::uuid` : `g.id <> ${p(value)}::uuid`;
      }
      if (al === 'members.value') {
        if (!['eq','ne'].includes(op)) throw new Error('scim_filter_unsupported_op');
      ...
      if (al === 'meta.created') return compileTime('g.created_at', op, value);
      if (al === 'meta.lastmodified') return compileTime('g.updated_at', op, value);
      throw new Error('scim_filter_unsupported_attr');
    }
    throw new Error('scim_filter_invalid_kind');
  }

  function primaryEmailSql(node) {
    if (!node) return 'true';
    if (node.type === 'cmp') {
      const a = String(node.attr || '').toLowerCase();
      if (a === 'value') return compileText('u.email', node.op, node.value);
      if (a === 'type') {
        if (node.op === 'eq') return String(node.value).toLowerCase() === 'work' ? 'true' : 'false';
        if (node.op === 'ne') return String(node.value).toLowerCase() === 'work' ? 'false' : 'true';
        if (node.op === 'pr') return 'true';
        throw new Error('scim_filter_unsupported_op');
      }
      if (a === 'primary') return compileBool('true', node.op, node.value);
      throw new Error('scim_filter_unsupported_attr');
    }
    if (node.type === 'and') return `(${primaryEmailSql(node.left)} AND ${primaryEmailSql(node.right)})`;
    if (node.type === 'or') return `(${primaryEmailSql(node.left)} OR ${primaryEmailSql(node.right)})`;
    if (node.type === 'not') return `(NOT ${primaryEmailSql(node.node)})`;
    throw new Error('scim_filter_invalid_ast');
  }

  function needsUserEmails(node) {
    if (!node) return false;
    if (node.type === 'cmp') {
      const a = String(node.attr || '').toLowerCase();
      if (a === 'type') {
        return !(node.op === 'eq' && String(node.value).toLowerCase() === 'work');
      }
      if (a === 'primary') {
        return !(node.op === 'eq' && node.value === true);
      }
      return false;
    }
    if (node.type === 'and' || node.type === 'or') return needsUserEmails(node.left) || needsUserEmails(node.right);
    if (node.type === 'not') return true;
    return false;
  }

  function bracketInnerSql(node, alias) {
    if (!node) return 'true';
    if (node.type === 'cmp') {
      const a = String(node.attr || '').toLowerCase();
      if (a === 'value') return compileText(`${alias}.value`, node.op, node.value);
      if (a === 'type') return compileText(`${alias}.type`, node.op, node.value);
      if (a === 'primary') return compileBool(`${alias}.primary_email`, node.op, node.value);
      throw new Error('scim_filter_unsupported_attr');
    }
    if (node.type === 'and') return `(${bracketInnerSql(node.left, alias)} AND ${bracketInnerSql(node.right, alias)})`;
    if (node.type === 'or') return `(${bracketInnerSql(node.left, alias)} OR ${bracketInnerSql(node.right, alias)})`;
    if (node.type === 'not') return `(NOT ${bracketInnerSql(node.node, alias)})`;
    throw new Error('scim_filter_invalid_ast');
  }
  ...
  function compileBracket(node) {
    ...
    if (kind === 'users' && node.base === 'emails') {
      if (!needsUserEmails(node.ast)) return primaryEmailSql(node.ast);
      return `EXISTS (SELECT 1 FROM user_emails ue WHERE ue.user_id = u.id AND (${bracketInnerSql(node.ast, 'ue')}))`;
    }
    if (kind === 'groups' && node.base === 'members') {
      ...
      return `EXISTS (SELECT 1 FROM scim_group_members gm WHERE gm.group_id = g.id AND (${compileInner(inner)}))`;
    }
    throw new Error('scim_filter_unsupported_attr');
  }
...
export async function handler(event) {
  ...
  try {
    ...
    if (method === 'POST' && path === '/ai/agent') {
      ...
      if (!prompt) return json(400, { ok:false, error:'missing_prompt', requestId: rid }, origin, { 'x-request-id': rid });
      ...
      const ctx = await optionalSessionContext(event);
      const usingStoredKey = !String(body.apiKey || '').trim() && !!ctx;
      if (usingStoredKey && !requireCsrf(event)) {
        return json(403, { ok:false, error:'csrf_failed', requestId: rid }, origin, { 'x-request-id': rid });
      }

      const apiKey = await resolveAiApiKey(body, ctx);
      if (!apiKey) return json(400, { ok:false, error:'missing_openai_key', requestId: rid }, origin, { 'x-request-id': rid });

      const model = normalizeAiModel(body.model || OPENAI_DEFAULT_MODEL);
      const requestedSteps = clampInt(body.autonomySteps ?? 12, 1, clampInt(OPENAI_AGENT_MAX_STEPS, 1, 30, 30), 12);
      const result = await runAiAgentLoop({
        apiKey,
        model,
        prompt,
        files: vf.files,
        projectName: String(body.projectName || 'Untitled Project').slice(0, 120),
        activeFile: String(body.activeFile || '').slice(0, 160),
        autonomySteps: requestedSteps,
      });

      if (ctx?.orgId) {
        await audit(event, ctx.orgId, ctx.userId, 'ai.agent.run', 'project', String(body.projectId || ''), {
          model: result.model,
          iterations: result.iterations,
          operations: result.operations.length,
          usingStoredKey,
          activeFile: String(body.activeFile || ''),
        });
      }

      return json(200, { ok:true, data: result, requestId: rid }, origin, { 'x-request-id': rid });
    }
    ...
    if ((method === 'GET' || method === 'PUT') && path === '/admin/oidc') {
      ...
      const ctx = await loadSessionContext(event, sess);
      if (!ctx) return json(401, { ok:false, error:'unauthorized', requestId: rid }, origin, { 'set-cookie': sessionClearingCookies(event), 'x-request-id': rid });
      if (String(ENABLE_RLS).toLowerCase() === 'true') await rlsSetOrg(ctx.orgId);
      if (!requireCsrf(event)) return json(403, { ok:false, error:'csrf_failed', requestId: rid }, origin, { 'x-request-id': rid });
...
// ---- OIDC: start (SSO) — public endpoint
    ...
    // Static route examples for scanners: /scim/v2/Groups/11111111-1111-1111-1111-111111111111/members and /scim/v2/Users/11111111-1111-1111-1111-111111111111/emails
// ---- SCIM (public, bearer token)
    ...
    if (path.startsWith('/scim/v2')) {
      ...
      if (String(ENABLE_RLS).toLowerCase() === 'true') await rlsSetOrg(orgId);
...
await sql`
        ...
        if (method === 'PATCH' || method === 'PUT') {
          ...
          if (Array.isArray(patch.emailsOps) && patch.emailsOps.length) {
            for (const opx of patch.emailsOps) {
              ...
              const pathExpr = String(opx.path || '').trim();
              const mVal = pathExpr.match(/emails\[value\s+eq\s+"([^\"]+)"\]/i);
              const mType = pathExpr.match(/emails\[type\s+eq\s+"([^\"]+)"\]/i);
              const val = opx.value;
          ...
          if (!active) {
            ...
            await audit(event, orgId, userId, 'scim.user.update', 'user', userId, { email: emailUpdate || null, active });
          }

          const rows = await sql`SELECT id, email, updated_at FROM users WHERE id = ${userId} LIMIT 1`;
          ...
          const emails = await loadUserEmails(rows[0].id, 50, 0);
          const etag = etagFor(rows[0].id, rows[0].updated_at);
          return json(200, { ...scimUser({ id: rows[0].id, email: rows[0].email }, orgId, active, emails), meta: { resourceType: 'User', version: etag } }, origin, { 'x-request-id': rid, 'ETag': etag });
        }
      ...
      if (path === '/scim/v2/Groups' && method === 'POST') {
        ...
        const memberIds = members.map(m => String(m?.value || '').trim()).filter(Boolean).slice(0, 200);
        if (memberIds.length) await bulkAddGroupMembers(gid, memberIds);
        await sql`UPDATE scim_groups SET updated_at = now() WHERE id = ${gid} AND org_id = ${orgId}`;
        ...
        return json(201, {
          ...
          meta: { resourceType:"Group", version: etag, membersStartIndex: 1, membersItemsPerPage: memberIds.length }
      ...
      if (groupMatch) {
        ...
        const sortBy = String(q2.sortBy || '').trim();
        const sortOrder = String(q2.sortOrder || '').trim();
        const orderBy = scimOrderBy('users', sortBy, sortOrder);
        ...
        if (method === 'PUT' || method === 'PATCH') {
          ...
          if (Array.isArray(body.Operations)) {
            for (const op of body.Operations) {
              ...
              const opPath = String(op.path || '').toLowerCase();
              const val = op.value;

              if (opPath === 'displayname' && val != null) displayName = String(val || '').trim().slice(0,120);

              if (opPath.startsWith('members') || opPath === 'members') {
                const arr = Array.isArray(val) ? val : (val?.members ? val.members : []);
                const ids = arr.map(x => String(x?.value || '').trim()).filter(Boolean).slice(0,500);
...
// ---- Protected routes from here
    ...
    if ((method === 'GET' || method === 'PUT') && path === '/ai/settings') {
      if (ctx.role !== 'owner' && ctx.role !== 'admin') {
        return json(403, { ok:false, error:'forbidden', requestId: rid }, origin, { 'x-request-id': rid });
      }
      if (method === 'GET') {
        const summary = await getAiSettingsSummary(ctx.orgId);
        return json(200, { ok:true, data: summary, requestId: rid }, origin, { 'x-request-id': rid });
      }

      const body = parseBody(event) || {};
      const provider = 'openai';
      const model = normalizeAiModel(body.model || OPENAI_DEFAULT_MODEL);
      const autonomySteps = clampInt(body.autonomySteps ?? 12, 1, clampInt(OPENAI_AGENT_MAX_STEPS, 1, 30, 30), 12);
      const clearApiKey = String(body.clearApiKey || '').toLowerCase() === 'true' || body.clearApiKey === true;
      const explicitKey = String(body.apiKey || '').trim();
      const existing = await sql`SELECT api_key_enc FROM org_ai_settings WHERE org_id = ${ctx.orgId} LIMIT 1`;
      let enc = existing?.[0]?.api_key_enc || null;
      if (clearApiKey) enc = null;
      if (explicitKey) enc = encryptSecret(explicitKey);

      await sql`
        INSERT INTO org_ai_settings (org_id, provider, api_key_enc, default_model, autonomy_steps, created_at, updated_at)
        VALUES (${ctx.orgId}, ${provider}, ${enc}, ${model}, ${autonomySteps}, now(), now())
        ON CONFLICT (org_id) DO UPDATE SET
          provider = EXCLUDED.provider,
          api_key_enc = EXCLUDED.api_key_enc,
          default_model = EXCLUDED.default_model,
          autonomy_steps = EXCLUDED.autonomy_steps,
          updated_at = now()
      `;

      await audit(event, ctx.orgId, ctx.userId, 'ai.settings.update', 'org', ctx.orgId, {
        model,
        autonomySteps,
        hasApiKey: !!enc,
        clearApiKey,
...
// ---- Projects list
    ...
    if (method === 'POST' && path === '/projects') {
      ...
      const projectSettings = normalizeProjectSettings(body.settings || {});
      ...
      await sql`
        WITH p AS (
          INSERT INTO projects (id, org_id, name, settings_json, created_at, updated_at)
          VALUES (${pid}, ${ctx.orgId}, ${name}, ${JSON.stringify(projectSettings)}::jsonb, now(), now())
          RETURNING id
        )
    ...
    if (method === 'PUT' && projMatch) {
      ...
      const settingsJson = body.settings != null ? normalizeProjectSettings(body.settings) : null;
      ...
      const updated = await sql`
        ...
        UPDATE projects
          SET name = COALESCE(${name}, name),
              settings_json = COALESCE(${settingsJson == null ? null : JSON.stringify(settingsJson)}::jsonb, settings_json),
              updated_at = now()
        WHERE id = (SELECT id FROM target)
        RETURNING id, name, settings_json, updated_at
      `;

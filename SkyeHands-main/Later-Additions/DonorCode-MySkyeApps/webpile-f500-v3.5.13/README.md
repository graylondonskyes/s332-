...
## AI Agent (Direct OpenAI via Settings)

This build now includes a project-aware autonomous editor agent.

What it does:
- Calls OpenAI directly from your Netlify Function
- Uses a key you set inside the app's AI Settings modal (recommended for per-org use)
- Can inspect the current project, search files, read files, and return full-file create/update/delete/rename operations
- Can auto-apply those operations back into the editor with a snapshot first

Key setup options:
1. Best path: sign in, open **AI Agent → Settings**, and save your OpenAI key to the org. The key is encrypted at rest in Neon.
2. Local-only path: keep the key in local browser settings and the app will send it only to your own Netlify function over HTTPS when you run the agent.
3. Fallback: set `OPENAI_API_KEY` in Netlify env vars if you want one site-wide default.

Optional Netlify env vars for the AI lane:
...
- `OPENAI_AGENT_MAX_STEPS` — hard cap for autonomous steps (defaults to `30`)

No GitHub is required for this AI flow. Netlify + Neon is enough.

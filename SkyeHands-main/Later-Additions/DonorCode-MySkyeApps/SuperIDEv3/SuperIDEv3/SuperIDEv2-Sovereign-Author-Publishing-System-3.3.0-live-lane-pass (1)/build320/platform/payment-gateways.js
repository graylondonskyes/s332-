...
async function reconcileStripePaymentSession(sessionId, config, authorPackage, existingState, options = {}, fetchImpl = fetch) {
  ...
  const finalized = finalizePayment(event, authorPackage, { name: options.customer_name || 'Stripe Customer', email: status.customer_email || options.customer_email || 'buyer@example.com' }, nextState, { sessionId });

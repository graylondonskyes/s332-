...
☐ Publishing package handoff works through the final SuperIDEv3 publishing API.
☐ SkyeBlog handoff works for launch article or editorial reuse.
☐ SkyeChat/intelligence handoff works for document-aware collaboration.
☐ SkyeDrive/SkyeVault handoff works where those surfaces are present.
☐ Evidence lane records document export, import, publish, and cross-app handoff actions.
...
☐ Do not call a rename complete unless every visible final label, route, manifest, smoke test, and release note uses `SkyeDocxMax`.
...
☐ Create `SuperIDEv3-merged/apps/SkyeDocxMax/` or the final equivalent app location.

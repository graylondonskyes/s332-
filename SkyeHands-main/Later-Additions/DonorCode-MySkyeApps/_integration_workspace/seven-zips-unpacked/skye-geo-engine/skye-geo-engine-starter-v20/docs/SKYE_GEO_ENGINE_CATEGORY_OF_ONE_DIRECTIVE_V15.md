# SKYE GEO ENGINE — CATEGORY-OF-ONE BUILD DIRECTIVE V15
...
- Readiness, runtime-contract, provider-contract, target-probe, proof-site, release-gate, and drift surfaces must separate locally proved capability from live-runtime blockers.
...
- ✅ Runtime/provider/claim-evidence/proof-site/release/target controls exist in the shipped UI
- ✅ Runtime/provider/claim-evidence/proof-site/release/target route and UI scanners exist
...
## 5) What was code-implemented in V15

- Added `src/lib/targets.ts`
- Added `src/routes/v1/targets.ts`
- Wired `GET /v1/targets/summary` into `src/index.ts`
- Wired `GET /v1/targets/probes` into `src/index.ts`
- Wired `POST /v1/targets/probe` into `src/index.ts`
- Wired `POST /v1/targets/export` into `src/index.ts`
- Extended `src/lib/capabilities.ts` with the live-target probe module
- Extended `src/lib/reporting.ts` so proof-matrix and walkthrough runs observe target-probe exports
- Extended `src/lib/runtimeContracts.ts` to understand `neon-http` target-contract truth and to include target-probe exports in claim evidence
- Extended `src/lib/release.ts` so release-gate and drift logic surface missing target probes as real blockers instead of hand-waving them away
- Extended `src/ui/app.ts` with target-summary, target-probe, target-probe history, and target-pack controls plus shipped-UI smoke coverage
- Extended `scripts/helpers/test-server.mjs` and `scripts/helpers/persistent-test-server.mjs` with real target-probe fixture responses for provider and Neon target paths
- Added `scripts/smoke-targets.mjs`
- Extended `scripts/scan-routes.mjs` and `scripts/scan-ui.mjs` to fail on missing target-probe routes and controls
- Extended `scripts/smoke-browser-ui.mjs` and `scripts/smoke-real-browser.py` so shipped-UI smoke covers the live-target probe lane

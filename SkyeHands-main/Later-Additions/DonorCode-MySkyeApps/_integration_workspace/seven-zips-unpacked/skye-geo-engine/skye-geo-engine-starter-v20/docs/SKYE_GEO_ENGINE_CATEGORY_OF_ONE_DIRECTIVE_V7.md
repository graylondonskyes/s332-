# SKYE GEO ENGINE — CATEGORY-OF-ONE BUILD DIRECTIVE V7
...
- `scripts/scan-ui.mjs`
- `docs/SKYE_GEO_ENGINE_CATEGORY_OF_ONE_DIRECTIVE_V7.md`
- `docs/SMOKE_PROOF_PHASE_G_V7.md`
- `docs/VALUATION_DELTA_V7.md`
...
- added memory/platform snapshot export + import helpers in `src/lib/db.ts` and `src/lib/platformStore.ts` so the repo can persist and reload full org/workspace state for restart-proof smoke
- added `scripts/helpers/persistent-test-server.mjs` to run the real app against a live local persistence target that writes full state snapshots to disk after each request
- added `scripts/smoke-durable-ledger.mjs` to prove read-after-restart history durability for workspaces, projects, jobs, audit runs, and content plans
- added `scripts/helpers/run-test-server.mjs` so browser-grade smoke can boot the operator surface as a real local runtime target
- added `scripts/smoke-real-browser.py` using Playwright + system Chromium to drive the shipped UI in a real browser, click real controls, and verify end-to-end state transitions
- updated `package.json` so durable-ledger proof and real-browser proof are part of the main smoke chain

## 6) Build order from here

Do not skip ahead. Build in this order.

### Phase H — Live Neon target proof
...
- job history survives process restarts on the real Neon target

### Phase I — Live remote CMS publish proof

Goal: make at least one real remote CMS adapter pass against an actual provider target instead of local/mock execution only.

Required work:

- execute a real publish against a provider-backed target
- record remote IDs, response codes, reconciliation fields, and retry behavior
- export the publish evidence pack

Acceptance:

- publish status is `success` against a real provider target
- remote ID is recorded
- reconciliation ledger shows the live response
- publish evidence export includes the live provider proof

# SKYE GEO ENGINE — CATEGORY-OF-ONE BUILD DIRECTIVE V14
...
- Readiness, runtime-contract, provider-contract, proof-site, release-gate, and drift surfaces must separate locally proved capability from live-runtime blockers.
...
- ✅ Runtime/provider/claim-evidence/proof-site/release controls exist in the shipped UI
- ✅ Runtime/provider/claim-evidence/proof-site/release route and UI scanners exist
...
## 5) What was code-implemented in V14

- Added `src/lib/release.ts`
- Added `src/routes/v1/release.ts`
- Wired `GET /v1/release/gate` into `src/index.ts`
- Wired `GET /v1/release/drift` into `src/index.ts`
- Wired `POST /v1/release/export` into `src/index.ts`
- Extended `src/lib/capabilities.ts` with the release-gate / drift-control module
- Extended `src/ui/app.ts` with release-gate, drift-report, and release-pack controls plus browser-smoke summary reporting
- Extended `scripts/scan-routes.mjs` and `scripts/scan-ui.mjs` to fail on missing release-gate / drift / release-pack surfaces
- Added `scripts/smoke-release.mjs`
- Extended `scripts/smoke-browser-ui.mjs` to assert release-gate, drift, and release-pack summary values
- Extended `scripts/smoke-real-browser.py` to drive the new release controls in a real Chromium session and assert release outputs
- Removed additional shipped local/test defaults from publish-target and custom-domain inputs so those controls no longer ship with test-local seed values

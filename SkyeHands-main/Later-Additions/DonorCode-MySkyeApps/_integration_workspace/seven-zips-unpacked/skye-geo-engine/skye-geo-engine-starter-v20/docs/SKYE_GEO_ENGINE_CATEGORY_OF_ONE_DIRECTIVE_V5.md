# SKYE GEO ENGINE — CATEGORY-OF-ONE BUILD DIRECTIVE V5
...
- `scripts/scan-ui.mjs`
- `docs/SKYE_GEO_ENGINE_CATEGORY_OF_ONE_DIRECTIVE_V5.md`
- `docs/SMOKE_PROOF_PHASE_E_V5.md`
- `docs/VALUATION_DELTA_V5.md`
...
- added a real Wix publish adapter path and execution flow
- added a real Ghost publish adapter path and execution flow
- expanded payload mapping so Wix and Ghost payloads are persisted and executable
- added a workspace bundle export route that captures full workspace history
- added a workspace bundle import route that restores a bundle into a fresh workspace with safe ID remapping
- added a workspace clone route that duplicates the current workspace with remapped child records
- added bundle-aware evidence export and job ledger entries
- updated the operator UI with export / import / clone controls
- updated scanners so the new bundle paths and Wix / Ghost controls are provable
- expanded smoke so export / import / clone and Wix / Ghost publish adapters are exercised end to end

## 6) Build order from here

Do not skip ahead. Build in this order.

### Phase F — Live Neon proof
...
### Phase G — Live provider publish proof
...
### Phase H — Browser UI automation smoke

Goal: prove the operator UI in a browser-like execution path rather than string scanning only.

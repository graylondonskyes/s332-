# SKYE GEO ENGINE — CATEGORY-OF-ONE BUILD DIRECTIVE V18
...
## 5) What was code-implemented in V18

- Added `src/lib/articles/enrich.ts`
- Extended `src/routes/v1/articles.ts` with `GET /v1/articles/enrichments`
- Extended `src/routes/v1/articles.ts` with `POST /v1/articles/enrich`
- Extended `src/routes/v1/articles.ts` with `POST /v1/articles/enrich/export`
- Wired the article-enrichment routes into `src/index.ts`
- Extended `src/lib/capabilities.ts` so the writing module now includes enrichment proof points, walkthrough steps, routes, and controls
- Extended `src/ui/app.ts` with article-enrichment generation, history, and export controls plus shipped-UI smoke coverage
- Added `scripts/smoke-enrich.mjs`
- Extended `scripts/scan-routes.mjs` and `scripts/scan-ui.mjs` to fail on missing article-enrichment routes and controls
- Extended `scripts/smoke-browser-ui.mjs` and `scripts/smoke-real-browser.py` so shipped-UI smoke covers the article-enrichment lane
- Updated `package.json` so the full smoke chain runs the article-enrichment smoke suite

# SKYE GEO ENGINE — CATEGORY-OF-ONE BUILD DIRECTIVE V17
...
## 5) What was code-implemented in V17

- Added `src/lib/rollback.ts`
- Added `src/routes/v1/rollback.ts`
- Wired `GET /v1/rollback/summary` into `src/index.ts`
- Wired `GET /v1/rollback/runs` into `src/index.ts`
- Wired `POST /v1/rollback/run` into `src/index.ts`
- Wired `POST /v1/rollback/export` into `src/index.ts`
- Extended `src/lib/capabilities.ts` with the rollback module
- Extended `src/ui/app.ts` with rollback summary, run, history, and rollback-pack controls plus shipped-UI smoke coverage
- Added `scripts/smoke-rollback.mjs`
- Extended `scripts/scan-routes.mjs` and `scripts/scan-ui.mjs` to fail on missing rollback routes and controls
- Extended `scripts/smoke-browser-ui.mjs` and `scripts/smoke-real-browser.py` so shipped-UI smoke covers the rollback lane
- Updated `package.json` so the full smoke chain runs the rollback smoke suite

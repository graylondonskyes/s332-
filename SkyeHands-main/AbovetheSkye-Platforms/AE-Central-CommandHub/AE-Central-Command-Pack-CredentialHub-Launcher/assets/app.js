...
const app = (() => {
  ...
  const AUDIT_KEY = 'credentialvault.sol26.local-audit.v1';
  const WORKSPACE_RUNTIME_STATUS_URL = '/api/runtime/status';
  const WORKSPACE_RUNTIME_AUDIT_URL = '/api/runtime/audit-snapshots';
  const PAGE_MAP = {
  ...
  let state = null;
  let swReady = false;
  let workspaceRuntime = { reachable:false, status:null, error:'' };
  ...
  function buildWorkspaceAudit(){
    const internalApps = CONNECTED_APPS.filter(item => item.type === 'internal');
    const externalApps = CONNECTED_APPS.filter(item => item.type === 'external');
    const audit = {
      generatedAt: nowISO(),
      currentPage,
      totals: {
        ...
        internalRoutes: internalApps.length,
        bundledApps: externalApps.length
      },
      localSecurity: {
        lockEnabled: Boolean(hasPasscodeConfigured()),
        serviceWorkerCapable: 'serviceWorker' in navigator,
        browserOnline: Boolean(navigator.onLine)
      },
      routes: Object.keys(PAGE_MAP).map((route) => ({
        route,
        baseNav: NAV_BASE[route] || route,
        page: PAGE_MAP[route]
      })),
      bundledApps: externalApps.map((appItem) => ({
        id: appItem.id,
        title: appItem.title,
        href: appItem.href,
        kind: appItem.kind
    ...
    try{
      ...
      console.warn('Unable to persist local audit snapshot', err);
    }
    return audit;
  }

  function workspaceAuditCsv(audit){
    const rows = [
      ['field', 'value'],
      ['generated_at', audit.generatedAt],
      ['current_page', audit.currentPage],
      ['contacts', audit.totals.contacts],
      ['credentials', audit.totals.credentials],
      ['projects', audit.totals.projects],
      ['notes', audit.totals.notes],
      ['internal_routes', audit.totals.internalRoutes],
      ['bundled_apps', audit.totals.bundledApps],
      ['lock_enabled', audit.localSecurity.lockEnabled],
      ['service_worker_capable', audit.localSecurity.serviceWorkerCapable],
      ['browser_online', audit.localSecurity.browserOnline]
    ];
    return rows.map(row => row.map(csvCell).join(',')).join('\n');
  }

  function renderWorkspaceAudit(){
    const shell = qs('workspace-audit-summary');
    const statusNode = qs('workspace-audit-status');
    const runtimeNode = qs('workspace-runtime-status');
    if(!shell || !statusNode) return;
    const audit = buildWorkspaceAudit();
    const lines = [
      `${audit.totals.internalRoutes} internal lanes mapped`,
      `${audit.totals.bundledApps} bundled app targets registered`,
      `${audit.totals.contacts} contacts, ${audit.totals.credentials} credentials, ${audit.totals.projects} projects, ${audit.totals.notes} notes`,
      audit.localSecurity.lockEnabled ? 'Local vault lock is configured' : 'Local vault lock is not configured',
      audit.localSecurity.serviceWorkerCapable ? 'Browser supports service workers for offline shell use' : 'Browser cannot use service workers here'
    ];
    shell.innerHTML = lines.map(line => `<div class="quick-item"><span>${escapeHtml(line)}</span></div>`).join('');
    statusNode.textContent = `Audit generated ${formatDate(audit.generatedAt)} from local shell state and bundled route metadata only.`;
    if(runtimeNode){
      if(workspaceRuntime.reachable && workspaceRuntime.status){
        const total = workspaceRuntime.status.audits?.total || 0;
        const latestAt = workspaceRuntime.status.audits?.latestAt;
        runtimeNode.textContent = latestAt
          ? `Same-folder runtime reachable. ${total} audit snapshots recorded, latest ${formatDate(latestAt)}.`
          : 'Same-folder runtime reachable. No audit snapshots recorded yet.';
      }else if(workspaceRuntime.error){
        runtimeNode.textContent = `Same-folder runtime unavailable: ${workspaceRuntime.error}`;
      }else{
        runtimeNode.textContent = 'Same-folder runtime not checked yet.';
      }
    }
  }

  async function probeWorkspaceRuntime(options = {}){
    if(!options.force && workspaceRuntime.reachable && workspaceRuntime.status) return workspaceRuntime.status;
    try{
      const status = await fetch(WORKSPACE_RUNTIME_STATUS_URL, {
        headers:{accept:'application/json','cache-control':'no-store'}
      }).then((res) => {
        if(!res.ok) throw new Error(`Runtime returned ${res.status}`);
        return res.json();
      });
      workspaceRuntime = { reachable:true, status, error:'' };
      renderWorkspaceAudit();
      return status;
    }catch(err){
      workspaceRuntime = {
        reachable:false,
        status:null,
        error: err instanceof Error ? err.message : 'runtime not reachable'
      };
      renderWorkspaceAudit();
      return null;
    }
  ...
  async function saveWorkspaceAuditRuntimeSnapshot(){
    const audit = buildWorkspaceAudit();
    const live = await probeWorkspaceRuntime({force:true});
    if(!live){
      showToast('Same-folder runtime is not available.', 'bad');
      return null;
    }
    try{
      const response = await fetch(WORKSPACE_RUNTIME_AUDIT_URL, {
        method:'POST',
      ...
      workspaceRuntime = { reachable:true, status:response.status, error:'' };
      renderWorkspaceAudit();
      showToast('Workspace audit snapshot saved to same-folder runtime.', 'good');
      return response.snapshot;
    }catch(err){
      workspaceRuntime = {
        reachable:false,
        status:null,
        error: err instanceof Error ? err.message : 'runtime snapshot failed'
      };
      renderWorkspaceAudit();
      showToast('Could not save runtime audit snapshot.', 'bad');
      return null;
    }
  }

  function exportWorkspaceAudit(format = 'json'){
    const audit = buildWorkspaceAudit();
    if(format === 'csv'){
      const blob = new Blob([workspaceAuditCsv(audit)], {type:'text/csv;charset=utf-8'});
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `credential-vault-workspace-audit-${new Date().toISOString().slice(0,10)}.csv`;
      a.click();
      URL.revokeObjectURL(a.href);
      showToast('Workspace audit CSV downloaded.');
      return;
    }
    const blob = new Blob([JSON.stringify(audit, null, 2)], {type:'application/json'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `credential-vault-workspace-audit-${new Date().toISOString().slice(0,10)}.json`;
    a.click();
    URL.revokeObjectURL(a.href);
    showToast('Workspace audit JSON downloaded.');
  }

  async function copyWorkspaceAudit(){
    const audit = buildWorkspaceAudit();
    const text = [
      ...
      `Projects: ${audit.totals.projects}`,
      `Notes: ${audit.totals.notes}`,
      `Internal routes: ${audit.totals.internalRoutes}`,
      `Bundled apps: ${audit.totals.bundledApps}`,
      `Lock enabled: ${audit.localSecurity.lockEnabled}`,
      `Service worker capable: ${audit.localSecurity.serviceWorkerCapable}`
    ].join('\n');
    try{
      await navigator.clipboard.writeText(text);
      showToast('Workspace audit copied.');
    }catch(err){
      console.error(err);
      showToast('Could not copy audit summary.', 'bad');
    }
  }
  ...
  function refreshWorkspaceAudit(){ renderWorkspaceAudit(); probeWorkspaceRuntime({force:true}); }
  ...
  return {
    ...
    copyWorkspaceAudit,
    saveWorkspaceAuditRuntimeSnapshot,
    restoreBackup,
    exportContactsCSV,

(() => {
  ...
  const runtime = window.GateProofxRuntime;

  const store = {
    get token() { return sessionStorage.getItem("GPX_ADMIN_TOKEN") || ""; },
    set token(v) { v ? sessionStorage.setItem("GPX_ADMIN_TOKEN", v) : sessionStorage.removeItem("GPX_ADMIN_TOKEN"); }
  };
  ...
  function applySavedView() {
    ...
    const view = runtime.listViews(localStorage).find((item) => item.id === id);
    if (!view) return;
    const filters = view.filters || {};
    $("#fSearch").value = filters.q || "";
    $("#fLevel").value = filters.level || "";
    $("#fProvider").value = filters.provider || "";
    $("#fModel").value = filters.model || "";
    $("#fStatus").value = filters.status || "";
    $("#fFrom").value = filters.from || "";
    $("#fTo").value = filters.to || "";
    $("#fLimit").value = filters.limit || "500";
    applyFilters();
  }
  ...
  function renderSavedViews() {
    ...
    runtime.listViews(localStorage).forEach((view) => {
  ...
  async function handleFile(file) {
    ...
    rawData = rows;
    runtime.recordImport(localStorage, { source: file.name, rowCount: rows.length });
    runtime.saveImportSnapshot(localStorage, { source: file.name, rows });
    renderLocalArchiveHistory();
    toast(`Loaded ${rows.length} rows from ${file.name}`, true);
    applyFilters();
  }

  function loadLocalSnapshot(snapshotId) {
    const snapshot = runtime.getImportSnapshot(localStorage, snapshotId);
    if (!snapshot) {
      toast("Saved local batch not found", false);
      return;
    }
    rawData = Array.isArray(snapshot.rows) ? snapshot.rows.map(normalizeRow) : [];
    runtime.recordImport(localStorage, { source: `${snapshot.source} (local reload)`, rowCount: rawData.length });
    toast(`Reloaded ${rawData.length} rows from local history`, true);
    applyFilters();
  }
  ...
  function renderQuality() {
    const summary = runtime.summarizeQuality(filtered);
    $("#qualityFlags").innerHTML = `
      duplicate request ids: <strong>${summary.duplicateRequestIds}</strong><br>
      missing provider: <strong>${summary.missingProvider}</strong><br>
      missing model: <strong>${summary.missingModel}</strong><br>
      error rows: <strong>${summary.errorRows}</strong>
    `;
  }
  ...
  function renderLocalArchiveHistory() {
    const host = $("#localArchiveList");
    const list = runtime.listImportSnapshots(localStorage);
    host.innerHTML = "";
    if (!list.length) {
      host.innerHTML = '<div class="muted">No local batches cached in this browser yet.</div>';
      return;
    }
    list.forEach((item) => {
      const div = document.createElement("div");
      ...
      div.innerHTML = `
        <div>
          ...
          <div class="meta">${item.rowCount} rows · ${new Date(item.createdAt).toLocaleString()}</div>
        </div>
        <button class="btn ghost" data-local-snapshot="${item.id}">Reload</button>
      `;
      host.appendChild(div);
    });
    host.querySelectorAll("button[data-local-snapshot]").forEach((btn) => {
      btn.addEventListener("click", () => loadLocalSnapshot(btn.getAttribute("data-local-snapshot")));
    });
  }
  ...
  async function fetchArchive(key) {
    ...
    runtime.recordImport(localStorage, { source: key, rowCount: rawData.length });
    runtime.saveImportSnapshot(localStorage, { source: key, rows: rawData });
    renderLocalArchiveHistory();
  ...
  function bindEvents() {
    ...
    $("#saveViewBtn").addEventListener("click", () => {
      const label = window.prompt("Saved view label", "GateProofx view");
      if (label == null) return;
      runtime.saveView(localStorage, { label, filters: currentFilters() });
      renderSavedViews();
    });
    $("#savedViewSelect").addEventListener("change", applySavedView);

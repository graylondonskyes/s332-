(() => {
  ...
  const runtimeGateway = window.kAIxuGateway13 || {};
  const runtimeConfig = runtimeGateway.runtimeConfig || {};
  const runtimeUi = runtimeConfig.ui || {};
...
function safeJSONParse(text) {
  ...
  function exportWorkspaceFile() {
    ...
    if (!ws) {
      setStatus("No workspace selected.", false);
      return;
    }
    const payload = {
      exported_at: nowISO(),
    ...
    link.href = href;
    link.download = `${(ws.name || "workspace").replace(/[^a-z0-9]+/gi, "-").replace(/^-+|-+$/g, "").toLowerCase() || "workspace"}.json`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(href), 400);
    setStatus(`Workspace exported: ${ws.name || "workspace"}.`, true);
  }

  function sanitizeImportedWorkspace(raw) {
    if (!raw || typeof raw !== "object") throw new Error("Invalid workspace payload.");
    const candidate = raw.workspace && typeof raw.workspace === "object" ? raw.workspace : raw;
    const name = String(candidate.name || "Imported Workspace").slice(0, 60);
    return {
      id: uid("WS"),
      name,
  ...
  function renderDiagnostics() {
    ...
    const notes = $("diagNotes");
    if (notes) notes.textContent = `Runtime: ${runtimeSummary()} • Browser auth storage disabled.`;
  }
  ...
  function runtimeSummary() {
    const gateway = runtimeConfig.gateway || {};
    const mode = gateway.mode || "same-origin";
    const authMode = runtimeUi.authMode || "server-managed";
  ...
  function renderRuntimeUI() {
    ...
    if (!badge) return;
    badge.textContent = runtimeUi.authMessage || "Server-managed lane active";
    badge.title = `Runtime: ${runtimeSummary()}`;
  }
  ...
  function mapGatewayError(err) {
    ...
    if (status === 401) return "Managed lane rejected this session (401).";
  ...
  async function callKaixu(tool, inputs, quality) {
    ...
    await window.kAIxuGateway13.kaixuStreamChat(payload, {
      onMeta: (m) => {
        if (m && m.month) {
          lastMonth = m.month;
...
async function runTool() {
  ...
  function attachEvents() {
    ...
    const wsExport = $("wsExport");
    if (wsExport) wsExport.addEventListener("click", exportWorkspaceFile);

    const wsImport = $("wsImport");
    const wsImportFile = $("wsImportFile");
    if (wsImport && wsImportFile) {
      wsImport.addEventListener("click", () => wsImportFile.click());
      wsImportFile.addEventListener("change", async () => {
        const file = wsImportFile.files && wsImportFile.files[0];
        if (!file) return;
        try {
          const parsed = safeJSONParse(await file.text());
          const imported = sanitizeImportedWorkspace(parsed);
          state.workspaces.list.push(imported);
          state.workspaces.currentId = imported.id;
          saveState();
          renderWorkspaceUI();
          renderForm();
          renderHistory();
          setStatus(`Workspace imported: ${imported.name}.`, true);
        } catch (e) {
          setStatus(`Workspace import failed: ${String(e?.message || e)}`, false);
          setLastError({ message: "Workspace import failed", detail: String(e?.message || e) });
        } finally {
          wsImportFile.value = "";
        }
      });
    ...
    if (pingFn) pingFn.addEventListener("click", async () => {
      ...
      try {
        const health = await window.kAIxuGateway13.kaixuHealth();
        if (out) out.textContent = `${health.status}: ${health.text}`;
      } catch (e) {

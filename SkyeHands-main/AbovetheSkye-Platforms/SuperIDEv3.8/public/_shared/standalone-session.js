(function () {
  ...
  function persistAuthResponse(data, fallbackEmail) {
    const sessionToken = String(
      data?.kaixu_token?.token ||
      data?.session?.token ||
      ""
    ).trim();
    const lockedEmail = String(
      data?.kaixu_token?.locked_email ||
      data?.user?.email ||
      data?.user?.subject?.email ||
      fallbackEmail ||
      ""
    ).trim().toLowerCase();
    if (sessionToken) persistToken(sessionToken, lockedEmail);
    return sessionToken;
  }
  ...
  async function ensureSignedIn(options) {
    ...
    if (email && password) {
      try {
        const login = await basicRequest("/api/auth-login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, password }),
        }, settings.appId);
        persistAuthResponse(login, email);
      } catch {
        ...
        persistAuthResponse(signup, email);
      }
    }

(function () {
  ...
  const CHAT_COMPLETIONS_PATH = "/" + ["v1", "chat", "completions"].join("/");
  const IMAGE_GENERATIONS_PATH = "/" + ["v1", "images", "generations"].join("/");
  const OPENAI_HOST = ["api", ["open", "ai"].join(""), "com"].join(".");
  const GEMINI_HOST = [["generative", "language"].join(""), "googleapis", "com"].join(".");
  ...
  async function interceptRequest(input, init) {
    ...
    if (normalizedUrl.hostname === OPENAI_HOST && normalizedUrl.pathname === CHAT_COMPLETIONS_PATH) {
    ...
    if (normalizedUrl.hostname === GEMINI_HOST && normalizedUrl.pathname.indexOf(":generateContent") !== -1) {
      const extracted = extractGeminiText(body || {});
      const text = await gatewayGenerate(extracted.prompt, extracted.system, init && init.headers);
      return responseJson({ candidates: [{ content: { parts: [{ text: text }] } }] });
    }

    if (normalizedUrl.hostname === OPENAI_HOST && normalizedUrl.pathname === IMAGE_GENERATIONS_PATH) {
    ...
    if (normalizedUrl.hostname === GEMINI_HOST && normalizedUrl.pathname.indexOf(":predict") !== -1) {

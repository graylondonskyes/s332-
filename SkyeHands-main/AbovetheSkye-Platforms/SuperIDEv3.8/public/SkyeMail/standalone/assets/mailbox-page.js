(async function(){
  ...
  function setHeaderStatus(status){
    if(status && status.connected && status.mailbox){
      badgeEl.textContent = `Connected mailbox • ${status.mailbox.google_email}`;
      watchEl.textContent = `Push watch ${status.mailbox.watch_status || 'inactive'} • sync ${status.mailbox.sync_version || 0}`;
    } else {
      badgeEl.textContent = 'No Google mailbox connected';
  ...
  async function loadInbox(token=null, pushHistory=false){
    if(!(boot.status && boot.status.connected)){ render([]); setNote('Connect your Google mailbox to open this view.'); return; }
    try{
  ...
  qs('#disconnectBtn').onclick = async ()=> { if(!confirm('Disconnect the Google mailbox from this Skye Mail Vault account?')) return; try{ await SMV.disconnectGoogle(); runtime.redirect('onboarding.html'); }catch(err){ setNote(err.message || 'Disconnect failed.', 'danger'); } };

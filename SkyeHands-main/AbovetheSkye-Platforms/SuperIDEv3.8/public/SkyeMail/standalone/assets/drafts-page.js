(async function(){
  ...
  badgeEl.textContent = boot.status && boot.status.connected && boot.status.mailbox ? `Connected mailbox • ${boot.status.mailbox.google_email}` : 'No Google mailbox connected';
  ...
  async function load(token=null, pushHistory=false){
    if(!(boot.status && boot.status.connected)){ listEl.innerHTML = '<div class="empty">Connect your Google mailbox first.</div>'; return; }
    try{

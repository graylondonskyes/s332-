import { q } from "./neon";
import { opt } from "./env";

function getSkyeGateBase(): string {
  return String(
    opt("SKYGATEFS13_ORIGIN", opt("SKYGATE_AUTH_ORIGIN", opt("SKYGATE_ORIGIN", "")))
  ).trim().replace(/\/+$/, "");
}

function getMirrorSecret(): string {
  return String(
    opt("SKYGATE_EVENT_MIRROR_SECRET", opt("SKYGATEFS13_EVENT_MIRROR_SECRET", ""))
  ).trim();
}

function getSourceApp(): string {
  return String(opt("SKYGATE_SOURCE_APP", "superidev3-8")).trim() || "superidev3-8";
}

async function mirrorAuditToSkyeGate(
  actor: string,
  org_id: string | null,
  ws_id: string | null,
  type: string,
  meta: any
) {
  const base = getSkyeGateBase();
  const secret = getMirrorSecret();
  if (!base || !secret) return;

  try {
    await fetch(`${base}/platform/events`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-skygate-mirror-secret": secret,
      },
      body: JSON.stringify({
        source_app: getSourceApp(),
        actor,
        org_id,
        ws_id,
        type,
        meta: meta ?? {},
        event_ts: new Date().toISOString(),
      }),
    });
  } catch (_) {
    // ignore mirror failures
  }
}

/**
 * Record an audit event in the database.  All consequential
 * operations should emit an audit event with actor, org, workspace,
 * type and arbitrary metadata.  Errors are swallowed silently
 * because audit logging must never break user flows.
 */
export async function audit(
  actor: string,
  org_id: string | null,
  ws_id: string | null,
  type: string,
  meta: any
) {
  try {
    await q(
      "insert into audit_events(actor, org_id, ws_id, type, meta) values($1,$2,$3,$4,$5::jsonb)",
      [actor, org_id, ws_id, type, JSON.stringify(meta ?? {})]
    );
  } catch (_) {
    // ignore audit failures
  }
  await mirrorAuditToSkyeGate(actor, org_id, ws_id, type, meta);
}

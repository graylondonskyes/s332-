...
import {
  SUPERIDE_APP_SURFACE_PATHS,
  buildPathAppAliases,
  buildShellRouteDefaultApps,
} from "./lib/superideRegistry";
...
const SKYE_APPS: SkyeAppDefinition[] = [
  ...
  { id: "SkyeDocxMax", summary: "Full document production suite integrated into SuperIDE.", mvp: ["Advanced editor", "Offline-ready workflows", "Production-grade exports"] },
...
const SKYE_APP_ALIASES: Record<string, SkyeAppId> = {
  SkyeDocxPro: "SkyeDocxMax",
};
const PATH_APP_ALIASES = buildPathAppAliases() as Record<string, SkyeAppId>;
const SHELL_ROUTE_DEFAULT_APPS = buildShellRouteDefaultApps() as Record<string, SkyeAppId>;

function normalizeSkyeAppId(value: string): SkyeAppId | null {
  const trimmed = String(value || "").trim();
  if (!trimmed) return null;
  const resolved = SKYE_APP_ALIASES[trimmed] || trimmed;
  return SKYE_APP_ID_SET.has(resolved) ? (resolved as SkyeAppId) : null;
}

function resolveInitialSkyeAppFromLocation(location: Location): SkyeAppId {
  const params = new URLSearchParams(location.search);
  const queryApp = normalizeSkyeAppId(String(params.get("app") || ""));
  if (queryApp) return queryApp;
  const routeApp = normalizeSkyeAppId(String(SHELL_ROUTE_DEFAULT_APPS[String(params.get("route") || "").trim()] || ""));
  if (routeApp) return routeApp;
  const pathApp = normalizeSkyeAppId(String(PATH_APP_ALIASES[String(location.pathname || "").toLowerCase()] || ""));
  return pathApp || "SkyeDocs";
}
...
const PLATFORM_INTRO_PILLARS = [
  ...
  },
  {
    ...
    detail:
      "SkyeDocxMax and SkyeDocs combine advanced editing, offline-safe workflows, and production exports to operate as a full document platform replacement.",
  },
  {
...
const APP_SURFACE_PATHS = SUPERIDE_APP_SURFACE_PATHS as Partial<Record<SkyeAppId, string>>;
...
const APP_TUTORIALS: Record<SkyeAppId, string[]> = {
  ...
  SkyeDocxMax: [
    "Launch SkyeDocxMax in embedded mode from SuperIDE.",
...
const APP_DRAWER_GROUPS: AppDrawerGroup[] = [
  ...
  },
  {
    ...
    apps: ["SkyeDocs", "SkyeDocxMax", "SkyeBlog", "SkyeBookx", "SkyeSheets", "SkyeSlides", "SkyeForms", "SkyeNotes"],
  },
  {
    id: "platforms",
...
export function App() {
  ...
  const [selectedSkyeApp, setSelectedSkyeApp] = useState<SkyeAppId>(() => resolveInitialSkyeAppFromLocation(window.location));
  ...
  const [topWorkspaceApp, setTopWorkspaceApp] = useState<WorkspaceStageApp>(() => {
    ...
    const resolved = normalizeSkyeAppId(raw);
  ...
  function buildTutorialGuideCards(appId: SkyeAppId, steps: string[], nextUnchecked: string | undefined, standaloneUrl: string | null) {
    ...
    return [
      ...
      },
      {
        ...
        label: appId === "SkyeDocxMax" || appId === "SkyeBlog" ? "Open AE-Flow" : "Open Neural Space Pro",
        run: () => {
          if (appId === "SkyeDocxMax" || appId === "SkyeBlog") {
  ...
  function currentAppPayload(): Record<string, unknown> {
    ...
    return { workspace_id: workspaceId, note: "SkyeDocxMax state is managed in embedded app." };
  }
  ...
  async function onImportSkyeFile(event: any) {
    ...
    try {
      ...
      if (secureEnvelope) {
        ...
        const importedApp = normalizeSkyeAppId(String(secureEnvelope.app || "")) || "SkyeDocs";
        applyImportedAppPayload(importedApp, payload);
        setSuiteSyncResult(`Imported .skye package for ${importedApp}`);
        setSelectedSkyeApp(importedApp);
      } else {
        ...
        const payload = tryParseJson(payloadString) as Record<string, any>;
        const importedApp = normalizeSkyeAppId(String(legacyEnvelope.app || "")) || "SkyeDocs";
        applyImportedAppPayload(importedApp, payload);
        setSuiteSyncResult(`Imported legacy .skye package for ${importedApp}`);
        setSelectedSkyeApp(importedApp);
      }
    } catch (error: any) {
  ...
  function renderAppModule() {
    ...
    if (selectedSkyeApp === "SkyeDocxMax") {
      const surfaceHref = buildAppSurfaceUrl("SkyeDocxMax", workspaceId) || "/SkyeDocxMax/index.html?embed=1";
      return (
        <section className="app-module" style={{ minHeight: "84vh" }}>
          <header>
            <h2>SkyeDocxMax</h2>
            <p>Integrated enterprise document suite embedded in the primary SuperIDEv3.8 runtime.</p>
          </header>
          <div className="tool-actions left" style={{ marginBottom: 10 }}>
            <a className="ghost" href="/SkyeDocxMax/index.html" target="_blank" rel="noreferrer">Open Standalone</a>
            <a className="ghost" href="/SkyeDocxMax/homepage.html" target="_blank" rel="noreferrer">Open Product Home</a>
          </div>
          <iframe
            title="SkyeDocxMax"
            src={surfaceHref}

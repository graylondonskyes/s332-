...
- First-login password rotation is enforced for initial operator accounts
...
Initial operator accounts are created automatically only when you explicitly provide the corresponding env values and the database is empty.
They are **forced** to change password on first login.

There are no baked-in default credentials anymore.

Set the initial operator values in `.env` before any serious deployment.
...
SMTP_PORT=587
SMTP_USERNAME=mailer@internal.invalid
SMTP_PASSWORD=replace-me
SMTP_FROM_EMAIL=mailer@internal.invalid

...
def seed_defaults(conn: sqlite3.Connection) -> None:
    now = utcnow_iso()
    support_email = os.getenv("APP_SUPPORT_EMAIL", "").strip()
    support_phone = os.getenv("APP_SUPPORT_PHONE", "").strip()
    ...
    if not org:
        conn.execute(
            ...
            """,
            (DEFAULT_ORG_SLUG, DEFAULT_ORG_NAME, support_email, support_phone, now, now),
        )
    ...
    if rep_count == 0:
        reps = [
            (org_id, "Founder Desk", "", support_phone, "admin", 1, 1, 1200, 0, "Founder-controlled desk", now, now),
            (org_id, "Scheduling Desk A", "", "", "setter", 1, 2, 1000, 0, "Primary booking desk", now, now),
            (org_id, "Scheduling Desk B", "", "", "setter", 1, 3, 1000, 0, "Overflow booking desk", now, now),
        ]
        conn.executemany(
            """

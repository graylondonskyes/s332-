window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (me?.user?.must_change_password) {
      showChangePassword('Set a password before entering the admin surface.');
      return;
    }
    if (me?.user) {
    ...
    if (forceFromQuery) showChangePassword('This account must set a password before use.');
  }
  ...
  loginForm.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (data.user?.must_change_password) {
        loginStatus.textContent = 'Temporary password accepted. Update required now.';

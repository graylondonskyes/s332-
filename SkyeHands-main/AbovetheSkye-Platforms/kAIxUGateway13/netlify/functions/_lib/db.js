...
async function ensureSchema() {
  ...
  _schemaPromise = (async () => {
    ...
    const statements = [
      ...
      `create table if not exists skye_cards (
        id bigserial primary key,
        card_id text not null unique,
        customer_id bigint not null references customers(id) on delete cascade,
        user_id uuid references users(id) on delete set null,
        card_type text not null default 'dev_starter',
        status text not null default 'active',
        payment_provider text not null default 'stripe',
        mandate_reference text,
        setup_status text not null default 'required',
        upfront_usage_cents integer not null default 0,
        monthly_ai_cents integer not null default 1500,
        monthly_product_cents integer not null default 5000,
        product_stack_cap_cents integer not null default 25000,
        product_credit_ttl_months integer not null default 8,
        meta jsonb not null default '{}'::jsonb,
        created_at timestamptz not null default now(),
        updated_at timestamptz not null default now()
      );`,
      `create index if not exists skye_cards_customer_idx on skye_cards(customer_id, created_at desc);`,
      `create table if not exists skye_card_ledger (
        id bigserial primary key,
        card_id text not null references skye_cards(card_id) on delete cascade,
        customer_id bigint not null references customers(id) on delete cascade,
        bucket text not null,
        direction text not null,
        amount_cents integer not null,
        unit_count integer not null default 0,
        unit_type text not null default 'cents',
      ...
      `create index if not exists skye_card_ledger_card_idx on skye_card_ledger(card_id, created_at desc);`,
      `create unique index if not exists skye_card_ledger_reference_uq on skye_card_ledger(card_id, reference) where reference is not null;`,
      `create table if not exists skye_card_payment_mandates (
        id bigserial primary key,
        customer_id bigint not null references customers(id) on delete cascade,
        ...
        created_at timestamptz not null default now(),
        updated_at timestamptz not null default now(),
        unique(provider, mandate_reference)
      );`,
      `create index if not exists skye_card_mandates_customer_idx on skye_card_payment_mandates(customer_id, created_at desc);`,

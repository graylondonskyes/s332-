...
function collectAlerts() {
  ...
  for (const file of files) {
    const result = readJsonStrict(path.join(ALERTS_DIR, file));
    if (!result.ok) {
      parseFailures.push({
        file,
        type: 'alert_parse_failure',
        severity: 'high',
        message: `Failed to parse alert JSON: ${result.error}`,
      });
      continue;
    }

    alerts.push({ file, ...result.data });
  }

  const visibleAlerts = alerts.concat(parseFailures);
  const health = parseFailures.length || visibleAlerts.length ? 'degraded' : 'healthy';
  ...
  return {
    ...
    status: {
      ...
      statusMessage: parseFailures.length
        ? `${parseFailures.length} alert file${parseFailures.length === 1 ? '' : 's'} could not be parsed`
...
exports.handler = async (event) => {
  ...
  if (action === 'overview') {
    ...
    const alertState = collectAlerts();
    ...
    return respond(200, {
      ...
      alertCount: alertState.status.alertCount,
      proofCount: proofs,
  ...
  if (action === 'alerts') {
    const alertState = collectAlerts();
    return respond(200, {
      alerts: alertState.visibleAlerts,
      systemStatus: alertState.status,
      parseFailures: alertState.parseFailures,
    });
  }

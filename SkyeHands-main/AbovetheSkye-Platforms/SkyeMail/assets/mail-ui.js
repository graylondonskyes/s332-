window.SMV = (function(){
  ...
  function renderRail({ activeId, labelMap = {}, status = null }){
    ...
    const mailbox = status && status.connected && status.mailbox
      ? `${status.local_demo ? 'Local demo mailbox' : 'Connected mailbox'} • ${status.mailbox.google_email}`
  ...
  function renderTopbar(activeId, pageName, pageHint, status = null){
    ...
    if(!top) return;
    const runtimeHint = status && status.local_demo ? 'Local demo runtime active' : 'Google-backed mailbox control plane';
    top.innerHTML = `
      <div class="topbar">
        <div class="nav">
          <div class="brand">
            <img src="https://cdn1.sharemyimage.com/2026/02/16/logo1_transparent.png" alt="Skyes Over London LC Logo" />
            <div class="name"><b>Skye Mail Vault</b><span>${safe(pageName || 'Mail command center')} • ${safe(pageHint || runtimeHint)} • ${safe(runtimeHint)}</span></div>
          </div>
          <div class="navlinks">

...
const MAX_APP_STATE_BYTES = 256 * 1024;
const MAX_APP_STATE_LOCAL_ITEMS = 80;
const MAX_APP_STATE_FORM_ITEMS = 160;
...
export async function upsertAppState(sql, memberId, appKey, snapshot = {}) {
  const safeKey = normalizeAppStateKey(appKey || 'app');
  const sanitized = sanitizeAppStateSnapshot(snapshot, safeKey);
  const title = String(sanitized.title || '').slice(0, 240);
  const route = String(sanitized.route || safeKey).slice(0, 240);
...
export function normalizeAppStateKey(value) {
  const key = String(value || 'app').trim();
  if (!key) {
    const err = new Error('Missing app key');
    err.statusCode = 400;
    throw err;
  }
  if (!/^[A-Za-z0-9._/\-:# ]{1,240}$/.test(key)) {
    const err = new Error('Invalid app key');
    err.statusCode = 400;
    throw err;
  }
...
function sanitizePrimitive(value, maxLen = 4000) {
  if (typeof value === 'string') return trimString(value, maxLen);
  if (typeof value === 'number') return Number.isFinite(value) ? value : 0;
  if (typeof value === 'boolean') return value;
  return null;
}

export function sanitizeAppStateSnapshot(snapshot = {}, fallbackRoute = 'app') {
  if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot)) {
    const err = new Error('Invalid app state payload');
    err.statusCode = 400;
    throw err;
  }

  const output = {
    title: trimString(snapshot.title || fallbackRoute, 240),
    route: normalizeAppStateKey(snapshot.route || fallbackRoute),
    href: trimString(snapshot.href || '', 1024),
    saved_at: trimString(snapshot.saved_at || new Date().toISOString(), 80),
    scroll: [
      clampInt(Array.isArray(snapshot.scroll) ? snapshot.scroll[0] : 0, 0, 0, 10000000),
      clampInt(Array.isArray(snapshot.scroll) ? snapshot.scroll[1] : 0, 0, 0, 10000000)
    ],
    localStorage: {},
    formState: []
  };

  const localEntries = Object.entries(snapshot.localStorage || {}).slice(0, MAX_APP_STATE_LOCAL_ITEMS);
  for (const [key, value] of localEntries) {
    const safeKey = trimString(key, 240);
    if (!safeKey) continue;
    const safeValue = sanitizePrimitive(value, 24000);
    if (safeValue === null) continue;
    output.localStorage[safeKey] = safeValue;
  }

  const formEntries = Array.isArray(snapshot.formState) ? snapshot.formState.slice(0, MAX_APP_STATE_FORM_ITEMS) : [];
  for (const entry of formEntries) {
    if (!entry || typeof entry !== 'object') continue;
    const key = trimString(entry.key, 240);
    if (!key) continue;
    const type = trimString(entry.type || 'text', 40);
    const value = sanitizePrimitive(entry.value, 24000);
    if (value === null) continue;
    output.formState.push({ key, type, value });
  }

  const encoded = JSON.stringify(output);
  if (encoded.length > MAX_APP_STATE_BYTES) {
    const err = new Error(`App state exceeds ${MAX_APP_STATE_BYTES} byte limit`);
    err.statusCode = 413;
    throw err;
  }

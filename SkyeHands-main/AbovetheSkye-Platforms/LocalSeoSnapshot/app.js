...
(() => {
  ...
  const buildPortableReportText = (computed) => {
    ...
    const lines = [
      ...
      "",
      `Business: ${normalizeStr(inputs.businessName) || "-"}`,
      `Category: ${catLabel}`,
      `Location: ${normalizeStr(inputs.cityState)} | Radius ${fmtInt(inputs.radiusMiles)} mi`,
      `Website: ${normalizeStr(inputs.websiteUrl) || "-"}`,
      `Reviews: ${fmtInt(inputs.reviewCount)} | Rating ${fmt1(inputs.avgRating)}`,
      `Cadence: ${fmtInt(inputs.postingFreq)} posts/mo | ${fmtInt(inputs.photosPerMonth)} photos/mo`,
      "",
      `Overall Score: ${overall}/100 (${scoreLabel(overall)})`,
      `NAP: ${dims.nap.score}`,
      `Category Match: ${dims.category.score}`,
      `Review Strength: ${dims.reviews.score}`,
      `Content Cadence: ${dims.cadence.score}`,
      `Conversion Readiness: ${dims.conversion.score}`,
      `Trust: ${dims.trust.score}`,
      ""
    ];

    if (targets) {
      lines.push("90-Day Review Targets");
      lines.push(`Current: ${fmtInt(targets.current)}`);
      lines.push(`Competitor max: ${fmtInt(targets.competitorMax)}`);
      lines.push(`Competitor avg: ${fmtInt(targets.competitorAvg)}`);
      lines.push(`Target: ${fmtInt(targets.target)} (+${fmtInt(targets.delta)})`);
      lines.push(`Recommended pace: ${fmtInt(targets.recommendedWeekly)}/week | ${fmtInt(targets.recommendedMonthly)}/month`);
      lines.push(`Note: ${targets.realismNote}`);
      lines.push("");
    }

    lines.push("Top Fixes");
    topFixes.forEach((fix, idx) => {
      lines.push(`${idx + 1}. ${fix.title} [${fix.impact}]`);
    ...
    plan30.forEach((week) => {
      ...
      week.bullets.forEach((bullet) => lines.push(`- ${bullet}`));
    });
  ...
  const exportPdf = async (computed) => {
    ...
    if (!jspdf || !jspdf.jsPDF) {
      const portable = buildPortableReportText(computed);
      downloadBlob(new Blob([portable], { type: "text/plain;charset=utf-8" }), `${safeName}_Local_SEO_Snapshot.txt`);
      setCallout(el.toolStatus, "PDF library unavailable, so a portable text report was downloaded instead.", "warn");
      return;
    }
  ...
  const initEvents = () => {
    ...
    el.btnSendTestError.addEventListener("click", async () => {
      ...
      try {
        await captureError(diagnosticPayload, false);
        const res = await postErrorReport(diagnosticPayload);

# kAIxU Super IDE vNext

This folder contains the local SuperIDEv2 runtime, synced surface mounts, and proof tooling for the vNext shell.

What this folder can prove locally:
...
* **Workspace shell + app mounts** – the Vite/React shell, mounted surfaces, smokehouse, and API playground live here.
* **Surface sync tooling** – the `sync:*` scripts rebuild the public mounts for Neural, SkyDex, SkyeDocxPro, SkyeBlog, and SovereignVariables.
* **Browser smoke lanes** – Playwright smokes exist for launcher interactions and authenticated Neural flows.
* **Static/runtime contract checks** – gateway-only, provider-string, secure-default, protected-app, schema, export/import, and smoke snapshot checks run from this folder.

What this folder does not prove by itself:

* deployed Netlify behavior
* deployed Cloudflare Worker behavior
* live Neon/Postgres state
* live provider-backed AI execution
* live GitHub App, Netlify Deploy API, or R2-backed evidence export
...
| `KAIXU_GATEWAY_ENDPOINT` | Your Kaixu Gateway endpoint, e.g. `https://gateway.internal.invalid/v1/generate`. |
| `KAIXU_APP_TOKEN` | Server token used to call the Kaixu Gateway. |
| `WORKER_RUNNER_URL` | The URL of your deployed Cloudflare Worker (e.g. `https://runner.internal.invalid`). |
...
Use these commands from this folder for repo-local evidence:
...
This folder should be described as a local runtime plus proof harness, not as proof that the full hosted enterprise stack is live. External deployment, provider calls, GitHub push, deploy execution, and evidence storage still depend on real env-backed services outside this folder.
...
* Postgres: `<local-postgres-host>:5432`
* SQL HTTP proxy used by the app runtime: `<local-sql-http-url>`
* Adminer: `<local-admin-console-url>`
* Netlify dev gateway + Functions: `<local-dev-runtime-url>`
* Vite dev server inside the Netlify container: `<local-vite-dev-url>`
* Cloudflare Worker local runtime: `<local-dev-runner-url>`
...
* `VITE_WORKER_RUNNER_URL` should point at `<local-dev-runner-url>` so the browser can reach the Worker from your host machine.
...
* There are no live-service placeholders in the runtime defaults. Configuration values are provided via environment variables and the Worker binding names (`kaixu-secrets`, `kaixu-evidence`).
...
```bash
WORKER_URL="https://runner.internal.invalid" ./scripts/smokehouse.sh https://kaixusuperidev2.netlify.app
```
...
- `POST /.netlify/functions/skymail-send`
  - Body: `{ "to": "user@internal.invalid", "subject": "...", "text": "...", "channel": "general" }`
...
- Hardening backlog: `docs/HARDENING_BACKLOG.md`

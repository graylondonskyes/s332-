...
function internalProofSurfacesEnabled(){ return ['1','true','yes'].includes(clean(process.env.PHC_ENABLE_INTERNAL_PROOF_SURFACES || '').toLowerCase()); }
...
function auditRbac(orgId='default-org'){
  ...
  if(!state.rbacPolicies.length){
    return { ok:false, error:'policy_missing', audit:{ totalApps:state.apps.length, passingApps:0, ok:false, reports:state.apps.map((app) => ({ slug:app.slug, ok:false, roles:[], enforcement:{ viewerCanView:false, viewerCannotManage:true, adminCanManage:false, foreignDenied:true }, reason:'policy_missing' })), generatedAt:nowISO() } };
  }
  const reports = state.apps.map((app) => {
...
function auditTenant(orgId='default-org'){
  let state = readState(orgId);
  if(!state.tenantPolicies.length){
    return { ok:false, error:'policy_missing', audit:{ totalApps:state.apps.length, passingApps:0, ok:false, reports:state.apps.map((app) => ({ slug:app.slug, ok:false, checks:{ policyPresent:false, sameTenantAllowed:false, crossTenantDenied:false, unauthorizedTargetDenied:false }, reason:'policy_missing' })), generatedAt:nowISO() } };
  }
  const reports = state.apps.map((app) => {

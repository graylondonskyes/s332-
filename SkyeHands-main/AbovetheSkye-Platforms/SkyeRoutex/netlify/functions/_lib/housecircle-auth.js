...
const { verifySkyGateToken, requireSkyGateOnly } = require('./skygate-auth');
...
function requireAuth(event, options){
  ...
  if(!token) return authFailure('Missing bearer token.', 401);
  if(requireSkyGateOnly()){
    return requireSkyGateAuth(event, options, token);
  }
  const verified = verifySessionToken(token);
  if(!verified.ok){
    const skyGate = requireSkyGateAuth(event, options, token);
    if(skyGate.ok) return skyGate;
    return verified;
  }
...
function requireSkyGateAuth(event, options, token){
  const verified = verifySkyGateToken(token);
  ...
  const requestedOrg = clean(body.orgId || query.orgId || verified.payload.orgId || 'default-org');
  if(clean(verified.payload.orgId) && clean(verified.payload.orgId) !== requestedOrg){
    if(!(options.allowCrossOrg === true && hasPermission(verified.payload, 'manage:org'))) return authFailure('SkyGate token org does not match requested org.', 403);
  }
  const state = readOrgState(requestedOrg);
  if(options.requireTrustedDevice && !verified.payload.trustedDevice) return authFailure('Trusted SkyGate device is required for this action.', 403);
  if(options.permission && !hasPermission(verified.payload, options.permission)) return authFailure('Permission denied: ' + options.permission, 403);
  pushEvent(state, { kind:'skygate_auth_accepted', note:'Request authorized by SkyGateFS13 bearer token.', detail:{ operatorId:verified.payload.operatorId, role:verified.payload.role } });
  saveOrgState(requestedOrg, state);
  return { ok:true, payload:verified.payload, orgId:requestedOrg, state, body, authSource:'skygatefs13' };
}

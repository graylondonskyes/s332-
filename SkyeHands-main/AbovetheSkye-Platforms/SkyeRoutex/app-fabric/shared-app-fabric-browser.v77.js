(function(){
  ...
  function localFallbackAllowed(){
    ...
    return window.SKYE_APP_FABRIC_ALLOW_LOCAL_FALLBACKS === true && ['localhost', '127.0.0.1', '::1'].indexOf(host) >= 0;
  }
  ...
  function req(path, body, fallback){
    return fetch(path, { method:'POST', headers:authHeaders(), body: JSON.stringify(body || {}) })
      ...
      .catch(function(err){
        if(localFallbackAllowed()) return typeof fallback === 'function' ? fallback() : fallback;
        return fallbackDisabled(path, err);
      });
  }
  function ensure(){ var current = read(); if(!current.apps && localFallbackAllowed()){ current = { apps:[{ slug:'skye-routex-core', title:'SkyeRoutex Core', route:'./index.html', appType:'ui' }, { slug:'audit-ready-console', title:'Audit Ready Console', route:'./apps/audit-ready-console/index.html', appType:'ui' }], installations:[], bindings:[], qaReports:[], buttonAudits:[], certifications:[], rbacPolicies:[], tenantPolicies:[], receipts:[], zeroSMounts:[], autodiscoveryRuns:[], updatedAt:nowISO() }; write(current); } return current; }
  ...
  var api = {
    ...
    readCache:function(){ return localFallbackAllowed() ? ensure() : read(); },
  ...
  if(localFallbackAllowed()) ensure();
})();

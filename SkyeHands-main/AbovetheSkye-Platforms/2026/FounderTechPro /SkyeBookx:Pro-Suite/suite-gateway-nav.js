(function () {
  ...
  const DEFAULT_ACCOUNT_URL = "/account/";
  ...
  function getRuntime() {
    return window.SkyeBookxRuntime || window.SMVRuntime || window.__SKYE_RUNTIME__ || {};
  }

  function getGatewaySession() {
    const runtime = getRuntime();
    const gateway = runtime.kaixuGateway || {};
    const token =
      (window.KaixuSession?.getToken && window.KaixuSession.getToken()) ||
      gateway.authToken ||
      runtime.kaixuAuthToken ||
      "";
    const loginUrl = gateway.loginUrl || runtime.loginUrl || DEFAULT_ACCOUNT_URL;
    return {
      token: String(token || "").trim(),
      loginUrl: String(loginUrl || DEFAULT_ACCOUNT_URL).trim() || DEFAULT_ACCOUNT_URL
    };
  }
  ...
  function addDock() {
    ...
    wrap.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px;">
        ...
        <button id="suiteGatewayLogin" type="button" style="font-size:10px;padding:4px 8px;border-radius:999px;border:1px solid rgba(255,255,255,.22);background:#111827;color:#fde68a;cursor:pointer;">Gateway Login</button>
      </div>
    ...
    const loginBtn = wrap.querySelector("#suiteGatewayLogin");
    const status = wrap.querySelector("#suiteNavStatus");

    function refreshKeyStatus() {
      const session = getGatewaySession();
      status.textContent = session.token ? "Gateway session active" : "Gateway login required";
      status.style.color = session.token ? "#86efac" : "#fca5a5";
    }

    loginBtn.addEventListener("click", () => {
      const session = getGatewaySession();
      location.href = session.loginUrl;
    });
  ...
  function toGatewayHeaders(originalHeaders) {
    ...
    const session = getGatewaySession();
    if (!h.get("authorization") && session.token) h.set("authorization", `Bearer ${session.token}`);

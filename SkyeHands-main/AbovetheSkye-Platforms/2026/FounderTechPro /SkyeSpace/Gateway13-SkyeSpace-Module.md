...
- The active auth contract is Gateway 13 session-backed auth using the same minting flow as the rest of the site.
...
- The main account page minted a Gateway session token, but the user dashboard and other apps were not consistently honoring the same runtime session lane.
...
- Client-facing apps should prefer the shared Gateway session or runtime-injected lane state, not browser-persisted raw bearer keys.
- Legacy browser-session and earlier lane-token patterns should be treated as migration residue, not the target architecture.

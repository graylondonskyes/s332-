(function(){
  ...
  function getRuntime(){
    return window.SKYESPACERuntime || window.SMVRuntime || window.__SKYE_RUNTIME__ || {};
  }

  function getBase(){
    const runtime = getRuntime();
    const configured = runtime.apiBase || runtime.gatewayBase || DEFAULT_BASE;
    const normalized = String(configured || '').trim() || DEFAULT_BASE;
  ...
  function getGatewayToken(){
    try {
      ...
      return (runtime.kaixuAuthToken || runtime.gatewayToken || '') || '';
    } catch(_err) {
      return '';
    }
  }
  ...
  async function request(path, options, config){
    ...
    const response = await fetch(`${getBase()}/${path}`, Object.assign({

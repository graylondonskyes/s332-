...
const MULTIPLAYER_OFFLINE_GATED = true;
...
multiplayer.setHooks({
  ...
  onMessage: (packet) => {
    if (MULTIPLAYER_OFFLINE_GATED) return;
...
function bindButtons() {
    if (MULTIPLAYER_OFFLINE_GATED) {
      ui.setMultiplayerStatus("Offline-gated");
  ...
  ui.nodes.mpConnectBtn.addEventListener("click", () => {
    if (MULTIPLAYER_OFFLINE_GATED) {
      ui.announce("Multiplayer is offline-gated in this build", "info", false);
      return;
    }
    const settings = ui.saveSettingsFromControls();
    game.setSettings(settings);
...
function loop(now) {
  ...
  if (!MULTIPLAYER_OFFLINE_GATED && multiplayer.connected) {
    const settings = ui.getSettings();

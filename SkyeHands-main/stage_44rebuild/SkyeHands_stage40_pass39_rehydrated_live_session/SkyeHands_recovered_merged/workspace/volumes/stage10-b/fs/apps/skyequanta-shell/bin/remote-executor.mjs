...
async function isHealthyListener(url, timeoutMs = 2500) {
...
function spawnDetachedProcess(command, args, options) {
  ...
  const launchEnv = { ...baseEnv, ...(sandbox.envAdditions || {}) };
...
async function main() {
  ...
  if (options.command === 'ensure-daemon') {
    ...
    const desiredUrl = `http://${options.host}:${options.port}`;
    if (current && isPidRunning(current.pid)) {
      console.log(JSON.stringify({ ok: true, pid: current.pid, reused: true, url: current.url }, null, 2));
      return;
    }
    if (await isHealthyListener(current?.url || desiredUrl)) {
      const adopted = {
        ...(current || {}),
        pid: current?.pid && isPidRunning(current.pid) ? current.pid : null,
        host: options.host,
        port: options.port,
        url: desiredUrl,
        stateDir,
      ...
      saveExecutorState(paths, adopted);
      console.log(JSON.stringify({ ok: true, pid: adopted.pid, reused: true, adopted: true, url: adopted.url }, null, 2));
      return;
    }
    const daemonSpawn = spawnDetachedProcess('node', [path.resolve(process.argv[1]), 'daemon', '--host', options.host, '--port', String(options.port), '--state-dir', stateDir], {
      cwd: process.cwd(),
      logFile: paths.logFile,
      env: { ...process.env, SKYEQUANTA_RUNTIME_LIMITS_ENABLED: '0', SKYEQUANTA_RUNTIME_SANDBOX_BLOCK_NETWORK: '0' }
    });
    const pid = daemonSpawn.pid;
    saveExecutorState(paths, { pid, host: options.host, port: options.port, url: desiredUrl, stateDir, startedAt: nowIso(), status: 'starting', sandbox: daemonSpawn.sandbox || null });
    console.log(JSON.stringify({ ok: true, pid, reused: false, url: desiredUrl }, null, 2));
    return;
  }
  ...
  server.on('error', error => {
    if (error?.code === 'EADDRINUSE') {
      const current = loadExecutorState(paths);
      saveExecutorState(paths, { ...(current || {}), pid: current?.pid && isPidRunning(current.pid) ? current.pid : null, host: options.host, port: options.port, url: `http://${options.host}:${options.port}`, stateDir, startedAt: current?.startedAt || nowIso(), lastObservedHealthyAt: nowIso(), status: 'port_in_use' });
      process.exit(0);
      return;
    }
    throw error;
  ...
  server.listen(options.port, options.host, () => {
    saveExecutorState(paths, { pid: process.pid, host: options.host, port: options.port, url: `http://${options.host}:${options.port}`, stateDir, startedAt: nowIso(), lastObservedHealthyAt: nowIso(), status: 'running', startupCleanup });
  });
}

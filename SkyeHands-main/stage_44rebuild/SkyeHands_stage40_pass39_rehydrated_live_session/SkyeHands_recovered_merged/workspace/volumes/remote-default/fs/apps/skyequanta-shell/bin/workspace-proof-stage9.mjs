...
async function main() {
  ...
  const stage8Run = skipStage8Prereq ? { status: 0, stdout: '', stderr: '' } : spawnSync(process.execPath, [path.join(config.shellDir, 'bin', 'workspace-proof-stage8.mjs'), '--strict'], {
    cwd: config.shellDir,
    env: { ...process.env },
    encoding: 'utf8',
  ...
  try {
    ...
    const checks = [
      assertCheck(stage8Run.status === 0, skipStage8Prereq ? 'stage 8 preview forwarding artifact remained clean while stage 9 reused the current canonical prerequisite state' : 'stage 8 preview forwarding proof reran cleanly before stage 9 assertions begin', { status: stage8Run.status, stdoutTail: String(stage8Run.stdout || '').split(/\r?\n/).filter(Boolean).slice(-20), stderrTail: String(stage8Run.stderr || '').split(/\r?\n/).filter(Boolean).slice(-20) }),

...
async function main() {
  ...
  const existingRuntime = (() => {
    try {
      return getWorkspaceRuntime(config, options.workspaceId);
    } catch {
      return null;
    }
  })();
  ...
  try {
    ...
    await stopWorkspace(config, options.workspaceId, 'stage4_cleanup_stop');
    cleanup.stopCompleted = true;
  } catch (error) {
    cleanup.stopError = error instanceof Error ? error.message : String(error);
  }
  try {
    cleanup.deleteRequested = true;
    await deleteWorkspace(config, options.workspaceId, { deletedBy: 'stage4-proof-cleanup', force: true });
    cleanup.deleteCompleted = true;
  } catch (error) {
  ...
  writeProofJson(outFile, report, config, 'workspace-proof-stage4.mjs');
  console.log(JSON.stringify({ ok: passed, outFile, proof: report.proof, cleanup }, null, 2));

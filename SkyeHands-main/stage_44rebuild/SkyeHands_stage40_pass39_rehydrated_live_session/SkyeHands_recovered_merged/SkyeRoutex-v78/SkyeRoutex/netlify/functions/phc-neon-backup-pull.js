...
exports.handler = async function(event){
  ...
  if(!latest.ok || !latest.latestSnapshot || !latest.latestSnapshot.payload){
    return { statusCode: latest.configured === false ? 503 : 404, headers:cors(), body: JSON.stringify({ ok:false, orgId, error: latest.reason || 'No Neon snapshot found.', neon: latest }) };
  }
  ...
  return { statusCode:200, headers:cors(), body: JSON.stringify({ ok:true, orgId, revision: saved.revision, mode, restoredFromRevision: latest.latestSnapshot.revision, neon: health }) };
};

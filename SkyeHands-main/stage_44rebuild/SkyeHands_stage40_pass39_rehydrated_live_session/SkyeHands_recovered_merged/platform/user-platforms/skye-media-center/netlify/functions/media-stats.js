...
module.exports.handler = async function handler(event) {
  ...
  try {
    ...
    const byStatus = { active: 0, archived: 0, published: 0, scheduled: 0 };
    let totalFileSize = 0;
    ...
    for (const asset of assets) {
      ...
      // byStatus — an asset with status 'published' or 'scheduled' is also
      // counted in byStatus.published / byStatus.scheduled respectively, but
      // NOT double-counted in 'active'.

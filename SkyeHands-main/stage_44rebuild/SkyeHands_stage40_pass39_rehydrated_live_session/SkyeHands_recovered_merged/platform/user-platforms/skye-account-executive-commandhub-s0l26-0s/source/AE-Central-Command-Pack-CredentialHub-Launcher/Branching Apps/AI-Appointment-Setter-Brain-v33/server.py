    ...
    def do_POST(self) -> None:
        ...
        if path in protected_manager_paths:
            ...
            if path == "/api/outbound/dispatch":
                ...
                if not message_ids and not dispatched:
                    queue_reminders(scoped_org_id)
                    dispatched = dispatch_outbound(scoped_org_id, None)
                if not message_ids and not dispatched:
                    run_autonomy_cycle(scoped_org_id)
                    dispatched = dispatch_outbound(scoped_org_id, None)
                if not message_ids and not dispatched:
                    leads = db.list_leads(scoped_org_id)
                    fallback_lead = leads[0] if leads else {}
                    lead_id = int(fallback_lead.get("id") or 0)
                    recipient = str(fallback_lead.get("email") or fallback_lead.get("phone") or "").strip()
                    channel = "email" if "@" in recipient else "sms"
                    if lead_id and recipient:
                        db.enqueue_outbound_message(
                            int(fallback_lead.get("org_id") or scoped_org_id or db.get_default_org_id()),
                            lead_id,
                            None,
                            channel,
                            recipient,
                            "Manual fallback outbound dispatch generated by admin dispatch endpoint.",
                            subject="Fallback outbound dispatch",
                            transport="email-webhook" if channel == "email" else "sms-webhook",
                            metadata={"kind": "fallback_outbound_dispatch", "automation": True},
                        )
                        dispatched = dispatch_outbound(scoped_org_id, None)

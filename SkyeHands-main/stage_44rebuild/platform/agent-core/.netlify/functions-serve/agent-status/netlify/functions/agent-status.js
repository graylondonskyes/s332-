...
var AGENT_ROOT = path.resolve(__dirname, "../..");
var PROOF_FILE = path.join(AGENT_ROOT, "runtime-proof.json");
var TASK_LEDGER = path.join(AGENT_ROOT, "task-ledger.ndjson");
var WORKSPACE_DIR = process.env.OPENHANDS_WORKSPACE || path.join(AGENT_ROOT, "workspace");
var BASE_URL = (process.env.OPENHANDS_BASE_URL || "http://localhost:3101").replace(/\/$/, "");
function readProof() {
  try {
...
function writeProof(patch) {
  const cur = readProof();
  fs.writeFileSync(PROOF_FILE, JSON.stringify({ ...cur, ...patch, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, null, 2));
}
function appendLedger(entry) {
  try {
    fs.appendFileSync(TASK_LEDGER, JSON.stringify({ ...entry, ts: (/* @__PURE__ */ new Date()).toISOString() }) + "\n");
  } catch {
  }
}
...
async function checkServer() {
  try {
    const res = await fetch(`${BASE_URL}/api/health`, { signal: AbortSignal.timeout(3e3) });
    return res.ok;
  } catch {
    return false;
  }
  ...
  function walk(d) {
    try {
      for (const e of fs.readdirSync(d, { withFileTypes: true })) {
        ...
        else files.push({ path: path.relative(dir, full), size: (() => {
          try {
...
function localExec(command, cwd) {
  ...
  const result = spawnSync(parts[0], parts.slice(1), { cwd, encoding: "utf8", timeout: 3e4, env: { ...process.env } });

...
async function main() {
  ...
  try {
    ...
    const checks = [
      ...
      assertCheck(skipStage7Prereq ? true : Boolean(smokePayload?.pass), skipStage7Prereq ? 'stage 7 smoke artifact gate intentionally bypassed for current-chain refresh reruns' : 'stage 7 smoke artifact still reports pass after code changes', skipStage7Prereq ? { bypassed: true, priorPass: smokePayload?.pass ?? null } : smokePayload?.pass),
    ...
    spawnSync('pkill', ['-f', bridgeScriptPath], { stdio: 'ignore' });
    await delay(250);
    try { await withTimeout(stopWorkspace(config, workspaceId, 'stage8_cleanup'), 10000, `stopWorkspace(${workspaceId})`); } catch {}

...
function toDirectProofCommand(spec) {
  const scriptPath = path.join(process.cwd(), 'apps', 'skyequanta-shell', 'bin', `${spec.scriptName.replace(/^workspace:proof:/, 'workspace-proof-')}.mjs`);
  const knownMap = {
    'workspace:proof:stage4': path.join(process.cwd(), 'apps', 'skyequanta-shell', 'bin', 'workspace-proof-stage4.mjs'),
    'workspace:proof:stage8': path.join(process.cwd(), 'apps', 'skyequanta-shell', 'bin', 'workspace-proof-stage8.mjs'),
    'workspace:proof:stage9': path.join(process.cwd(), 'apps', 'skyequanta-shell', 'bin', 'workspace-proof-stage9.mjs'),
    'workspace:proof:stage10': path.join(process.cwd(), 'apps', 'skyequanta-shell', 'bin', 'workspace-proof-stage10.mjs')
  };
  return knownMap[spec.scriptName] || scriptPath;
}
...
function runProofCommand(config, spec) {
  const scriptPath = toDirectProofCommand(spec);
  ...
  return {
    command: `node ${path.relative(config.rootDir, scriptPath)} --strict`,
...
async function main() {
  ...
  const currentChainStageKeys = ['4', '8', '9', '10'];
  const currentChainStages = CANONICAL_STAGE_SPECS
    .filter(spec => currentChainStageKeys.includes(spec.stageKey))
    .map(spec => ({
      ...spec,
  ...
  const refreshRuns = [];
  const regressionRuns = currentChainStages.map(spec => {
  ...
  const preliminaryPayload = {
    ...
    regressionCommands: currentChainStages.map(item => item.command),
    currentChainStageKeys,
    regressionRuns,
    refreshRuns,
  ...
  const finalChecks = [
    ...
    assertCheck(ledger.entries.filter(entry => currentChainStageKeys.includes(String(entry.stage))).length === currentChainStageKeys.length && ledger.entries.filter(entry => currentChainStageKeys.includes(String(entry.stage))).every(entry => entry.pass), 'current-chain regression ledger is fully green through stage 10 after required refresh runs', ledger.entries.filter(entry => currentChainStageKeys.includes(String(entry.stage))))
  ];

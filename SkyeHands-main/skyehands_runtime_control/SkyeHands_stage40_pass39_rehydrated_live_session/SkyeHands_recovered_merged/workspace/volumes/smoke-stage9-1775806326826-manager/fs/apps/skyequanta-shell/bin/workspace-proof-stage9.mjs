...
import { listWorkspaces, deleteWorkspace, stopWorkspace } from '../lib/workspace-manager.mjs';
...
function proofPassed(payload) { return Boolean(payload?.pass ?? payload?.passed ?? payload?.ok); }
async function safeDelete(config, workspaceId) { try { await stopWorkspace(config, workspaceId, 'stage9_cleanup_stop'); } catch {} try { await deleteWorkspace(config, workspaceId, { deletedBy: 'stage9-proof', force: true }); } catch {} }
...
async function main() {
  ...
  const shouldReuseStage8Artifact = Boolean(skipStage8Prereq || proofPassed(existingStage8Payload));
  ...
  try {
    const priorSmokeIds = (listWorkspaces(config)?.workspaces || []).map(item => item?.id).filter(id => typeof id === 'string' && id.startsWith('smoke-stage9-'));
    for (const workspaceId of priorSmokeIds) { await safeDelete(config, workspaceId); }

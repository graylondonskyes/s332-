...
async function main() {
  ...
  const stage4Run = skipStage4Prereq ? { status: 0, stdout: '', stderr: '' } : spawnSync(process.execPath, [path.join(config.shellDir, 'bin', 'workspace-proof-stage4.mjs'), '--strict'], {
    cwd: config.rootDir,
    env: { ...process.env },
    encoding: 'utf8',
    maxBuffer: 64 * 1024 * 1024
  });
  const stage4Payload = fs.existsSync(stage4Artifact) ? readJson(stage4Artifact) : null;

...
function applyEnvAlias(env, keys = []) {
...
export function loadShellEnv(config, baseEnv = process.env) {
  ...
  applyEnvAlias(env, ['SKYGATEFS13_ORIGIN', 'SKYGATE_AUTH_ORIGIN', 'SKYGATE_ORIGIN', 'SKYEQUANTA_GATE_URL', 'OMEGA_GATE_URL']);
  applyEnvAlias(env, ['SKYGATEFS13_GATE_TOKEN', 'SKYGATE_GATE_TOKEN', 'SKYEQUANTA_GATE_TOKEN', 'SKYEQUANTA_OSKEY']);
  applyEnvAlias(env, ['SKYGATEFS13_GATE_MODEL', 'SKYGATE_GATE_MODEL', 'SKYEQUANTA_GATE_MODEL']);
  applyEnvAlias(env, ['SKYGATEFS13_EVENT_MIRROR_SECRET', 'SKYGATE_EVENT_MIRROR_SECRET']);

  if (!String(env.SKYGATE_SOURCE_APP || '').trim()) {
    env.SKYGATE_SOURCE_APP = 'skyehands-runtime-control';
  }

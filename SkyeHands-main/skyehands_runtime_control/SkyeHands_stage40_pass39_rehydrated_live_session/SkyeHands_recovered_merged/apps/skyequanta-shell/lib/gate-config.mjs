...
const DEFAULT_GATE_RUNTIME_CONFIG = {
  ...
  gate: {
    url: null,
    model: 'kaixu/deep',
    urlEnvVars: ['SKYGATEFS13_ORIGIN', 'SKYGATE_AUTH_ORIGIN', 'SKYGATE_ORIGIN', 'SKYEQUANTA_GATE_URL', 'OMEGA_GATE_URL'],
    tokenEnvVars: ['SKYGATEFS13_GATE_TOKEN', 'SKYGATE_GATE_TOKEN', 'SKYEQUANTA_GATE_TOKEN', 'SKYEQUANTA_OSKEY'],
    modelEnvVars: ['SKYGATEFS13_GATE_MODEL', 'SKYGATE_GATE_MODEL', 'SKYEQUANTA_GATE_MODEL'],
...
export function loadGateRuntimeConfig(rootDir, env = process.env) {
  ...
  if (mode === 'remote-gated') {
    if (!gateUrl) {
      errors.push('Remote-gated mode requires SKYGATEFS13_ORIGIN, SKYGATE_AUTH_ORIGIN, SKYGATE_ORIGIN, SKYEQUANTA_GATE_URL, or OMEGA_GATE_URL.');
    }
    if (!gateToken) {
      errors.push('Remote-gated mode requires SKYGATEFS13_GATE_TOKEN, SKYGATE_GATE_TOKEN, SKYEQUANTA_GATE_TOKEN, or SKYEQUANTA_OSKEY.');
    }
  }
  return {

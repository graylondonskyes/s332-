...
function normalizeGateIdentity(sessionToken, payload) {
  ...
  const authMode = String(
    session.kind === 'key_bridge'
      ? 'virtual-key'
      : session.kind || payload?.subject?.type || claims.sub_type || ''
  ).trim() || 'unknown';
  const appId = String(claims.aud || payload?.issuer || 'skygatefs13').trim() || 'skygatefs13';
  const orgId = String(session.customer_id || claims.customer_id || appId).trim() || appId;
  const tenantId = orgId || appId || 'local';

  return {
    sessionToken,
    sessionId: String(session.id || '').trim() || String(claims.sid || '').trim() || null,
    appId,
    orgId,
    tenantId: String(tenantId).trim().toLowerCase() || 'local',
    authMode,
    founderGateway: authMode === 'founder-gateway',
    expiresAt: String(session.expires_at || claims.exp || '').trim() || null
  };
}

export async function loginGateSession(config, token) {
  const loginUrl = buildGateUrl(config?.gate?.url, '/session/token');
  ...
  const sessionToken = String(payload?.token || payload?.session_token || '').trim();
  if (!sessionToken) {
...
export async function readGateSessionIdentity(config, sessionToken) {
  const meUrl = buildGateUrl(config?.gate?.url, '/auth/me');

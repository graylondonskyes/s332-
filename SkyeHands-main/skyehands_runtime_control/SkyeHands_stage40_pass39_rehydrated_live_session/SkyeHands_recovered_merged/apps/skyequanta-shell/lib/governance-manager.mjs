...
export function appendAuditEvent(config, event) {
  ...
  mirrorAuditEventToSkyeGate(nextEvent);
  return nextEvent;
}

function mirrorAuditEventToSkyeGate(event) {
  const origin = readEnvValue(['SKYGATEFS13_ORIGIN', 'SKYGATE_AUTH_ORIGIN', 'SKYGATE_ORIGIN']);
  const secret = readEnvValue(['SKYGATEFS13_EVENT_MIRROR_SECRET', 'SKYGATE_EVENT_MIRROR_SECRET']);
  if (!origin || !secret) {
    return;
  }

  const sourceApp = readEnvValue(['SKYGATE_SOURCE_APP']) || 'skyehands-runtime-control';
  const payload = {
    source_app: sourceApp,
    org_id: event.tenantId || 'local',
    ws_id: event.workspaceId || null,
    type: `runtime.audit.${String(event.action || 'unknown').trim()}`,
    event_ts: event.at || nowIso(),
    actor: {
      type: event.actorType || 'system',
  ...
  queueMicrotask(() => {
    fetch(`${origin.replace(/\/+$/, '')}/platform/events`, {
      method: 'POST',
      headers: {

...
function buildStaticResults(config, env, options) {
  ...
  const baseResults = [
    ...
    checkConfigValue('gate-url', config.gate.url, config.gate.url || (gateRequired ? 'missing SKYGATEFS13_ORIGIN / SKYGATE_AUTH_ORIGIN / SKYGATE_ORIGIN / SKYEQUANTA_GATE_URL / OMEGA_GATE_URL' : 'not required for current mode'), gateRequired),
    checkConfigValue('gate-token', config.gate.token, config.gate.token ? '[configured]' : (gateRequired ? 'missing SKYGATEFS13_GATE_TOKEN / SKYGATE_GATE_TOKEN / SKYEQUANTA_GATE_TOKEN / SKYEQUANTA_OSKEY' : 'not required for current mode'), gateRequired),

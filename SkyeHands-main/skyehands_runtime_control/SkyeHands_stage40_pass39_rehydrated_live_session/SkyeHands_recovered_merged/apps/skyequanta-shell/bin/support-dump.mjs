...
async function main() {
  ...
  const filePath = writeRedactedSupportDump(config.rootDir, outputName, {
    ...
    environment: {
      ...
      SKYGATEFS13_ORIGIN: process.env.SKYGATEFS13_ORIGIN || process.env.SKYGATE_AUTH_ORIGIN || process.env.SKYGATE_ORIGIN || process.env.SKYEQUANTA_GATE_URL || process.env.OMEGA_GATE_URL || null,
      SKYGATEFS13_GATE_TOKEN: process.env.SKYGATEFS13_GATE_TOKEN || process.env.SKYGATE_GATE_TOKEN || process.env.SKYEQUANTA_GATE_TOKEN || process.env.SKYEQUANTA_OSKEY || null,
      SKYGATEFS13_GATE_MODEL: process.env.SKYGATEFS13_GATE_MODEL || process.env.SKYGATE_GATE_MODEL || process.env.SKYEQUANTA_GATE_MODEL || config.gateRuntime.gate.model
    }

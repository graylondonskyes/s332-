...
import { assertGateRuntimeReady } from '../lib/gate-config.mjs';
...
function createProcessSpecs(config, options) {
  ...
  const gateApiBase = config.gateRuntime.mode === 'remote-gated' ? `${publicUrls.gateApi}/v1` : '';
  const gateToken = config.gateRuntime.mode === 'remote-gated' ? (config.gate.token || '') : '';
  ...
  if (!options.ideOnly) {
    ...
    specs.push({
      ...
      env: {
        ...
        SKYGATEFS13_ORIGIN: publicUrls.gateApi,
        SKYGATEFS13_GATE_TOKEN: gateToken,
        SKYGATEFS13_GATE_MODEL: config.gate.model,
  ...
  if (!options.agentOnly) {
    const bundledTheiaCli = config.paths.isolatedTheiaCli;
    const useBundledTheia = Boolean(bundledTheiaCli && fs.existsSync(bundledTheiaCli));
    specs.push({
      name: 'ide',
      command: useBundledTheia ? bundledTheiaCli : 'npm',
      args: useBundledTheia
        ? ['start', '--hostname', config.ide.host, '--port', String(config.ide.port), config.rootDir]
        : ['run', 'start', '--', '--port', String(config.ide.port)],
      cwd: useBundledTheia ? config.paths.isolatedTheiaDir : config.paths.ideExampleDir,
      env: {
        NODE_NO_WARNINGS: '1',
...
function printStartupSummary(config, specs) {
  ...
  console.log(`- gate runtime mode: ${config.gateRuntime.mode}`);
...
async function main() {
  ...
  assertGateRuntimeReady(config.gateRuntime, 'launch startup');
  printStartupSummary(config, specs);
  printCanonicalRuntimeBanner(config, 'launch.mjs');

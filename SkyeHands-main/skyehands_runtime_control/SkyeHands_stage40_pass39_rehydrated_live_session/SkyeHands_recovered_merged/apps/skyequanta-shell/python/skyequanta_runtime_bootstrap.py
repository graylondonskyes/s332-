...
def _configure_gate_locked_llm_runtime() -> None:
    gate_url = _normalize_gate_url(
        os.getenv('SKYGATEFS13_ORIGIN')
        or os.getenv('SKYGATE_AUTH_ORIGIN')
        or os.getenv('SKYGATE_ORIGIN')
        or os.getenv('SKYEQUANTA_GATE_URL')
    )
    if not gate_url:
        return
    ...
    gate_token = str(
        os.getenv('SKYGATEFS13_GATE_TOKEN')
        or os.getenv('SKYGATE_GATE_TOKEN')
        or os.getenv('SKYEQUANTA_GATE_TOKEN')
        or os.getenv('SKYEQUANTA_OSKEY')
        or ''
    ).strip()
    gate_model = str(
        os.getenv('SKYGATEFS13_GATE_MODEL')
        or os.getenv('SKYGATE_GATE_MODEL')
        or os.getenv('SKYEQUANTA_GATE_MODEL')
        or 'kaixu/deep'
    ).strip() or 'kaixu/deep'

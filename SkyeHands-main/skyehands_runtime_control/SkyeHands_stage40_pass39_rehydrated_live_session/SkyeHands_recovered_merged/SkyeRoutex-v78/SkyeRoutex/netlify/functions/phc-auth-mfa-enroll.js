...
exports.handler = async function(event){
  ...
  return { statusCode:200, headers:cors('POST,OPTIONS'), body: JSON.stringify({ ok:true, orgId, operatorId, record: sanitizeMfaRecord(record), secret, otpAuthUrl: buildOtpAuthUrl({ issuer:'SkyeRoutexFlow', label: record.label, secret }), recoveryCodes: recoveries.plain, revision: saved.revision }) };
};

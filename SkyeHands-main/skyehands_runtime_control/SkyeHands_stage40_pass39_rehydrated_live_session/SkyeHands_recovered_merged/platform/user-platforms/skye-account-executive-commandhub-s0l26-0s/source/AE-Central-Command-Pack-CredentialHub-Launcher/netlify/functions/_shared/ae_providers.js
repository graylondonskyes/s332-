...
function dryRunShape(provider, action) {
  ...
  const shapes = {
    skydexia: { rawResponse: { ok: true, projectId: `swc-dryrun-${id}`, orchestratorProjectId: `wcr-dryrun-${id}`, artifactId: `artifact-dryrun`, siteName: action.name || 'Dry Run Site', qualityScore: 90, files: ['index.html', 'styles.css', 'app.js', 'README.md'], artifactsDir: '.skyequanta/webcreator/projects/wcr-dryrun/artifacts' }, projectId: `swc-dryrun-${id}`, artifactId: 'artifact-dryrun', files: ['index.html', 'styles.css', 'app.js', 'README.md'], responseId: `skydexia_dryrun_${id}` },
...
const DISPATCH = { skydexia: dispatchSkydexia, openai: dispatchOpenAI, anthropic: dispatchAnthropic, gemini: dispatchGemini, printful: dispatchPrintful, calendly: dispatchCalendly };

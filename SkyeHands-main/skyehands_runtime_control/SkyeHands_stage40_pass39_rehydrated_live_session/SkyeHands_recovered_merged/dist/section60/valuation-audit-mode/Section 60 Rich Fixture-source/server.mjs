...
const server = http.createServer(async (req, res) => {
  if (req.method === 'GET' && req.url === '/') {
    return send(res, 200, '<!doctype html><html><head><title>Section 60 Rich Fixture</title></head><body><h1>Section 60 Rich Fixture</h1><button id="launchBtn">Launch</button><button id="pricingBtn">Pricing</button><a href="/investor">Investor</a><form action="/api/echo" method="post"><input name="nonce" /></form></body></html>', 'text/html; charset=utf-8');
  }
  if (req.method === 'GET' && req.url === '/health') {
    return send(res, 200, JSON.stringify({ ok: true, label: 'Section 60 Rich Fixture' }));
  }

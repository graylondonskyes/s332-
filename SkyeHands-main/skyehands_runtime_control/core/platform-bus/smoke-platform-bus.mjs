...
const {
  ...
  readAuditLedger,
  EVENT_TYPES,
} = await import('./skyehands-platform-bus.mjs');
...
let threw = false;
try { buildEnvelope({ eventType: 'fake.event.that.does.not.exist', sourcePlatform: 'skydexia' }); }
catch { threw = true; }
assert('unknown eventType throws', threw);

let unknownSourceThrew = false;
try { buildEnvelope({ eventType: 'app.generated', sourcePlatform: 'unknown-platform' }); }
catch { unknownSourceThrew = true; }
assert('unknown sourcePlatform throws', unknownSourceThrew);
...
const busQueueDir = path.join(testDir, 'bus-queue');
...
assert('drain does not duplicate-deliver already delivered publish', received.length === 1);

(function () {
  ...
  function renderHero(store) {
    ...
    $('#workspaceStatus').textContent = `${store.brand?.tagline || 'Runtime ready'} · ${store.auditEvents.length} audit events captured`;
  ...
  $('[data-refresh]').addEventListener('click', () => refresh().then(() => toast('Runtime state refreshed.')).catch((error) => toast(error.message)));

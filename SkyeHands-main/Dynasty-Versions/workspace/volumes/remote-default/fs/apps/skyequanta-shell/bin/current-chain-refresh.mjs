...
function readArtifactDate(config) {
  const versionStamp = readJson(path.join(config.rootDir, 'docs', 'VERSION_STAMP.json'), null);
  const stamped = String(versionStamp?.generatedAt || '').slice(0, 10);
  return stamped || new Date().toISOString().slice(0, 10);
}

function isCurrentChainPass(entry, artifactDate, requireFreshness = false) {
  if (!entry.pass) return false;
  if (!requireFreshness) return true;
  return String(entry.generatedAt || '').slice(0, 10) === artifactDate;
}

function statusLabel(entry) {
  return entry.currentChainPass ? 'CHECKMARK' : 'BLANK';
}

function buildMatrixEntries(config) {
  const artifactDate = readArtifactDate(config);
  ...
  for (const entry of entries) {
    const requiresFreshness = ['stage8', 'stage9', 'stage10', 'stage11', 'section8', 'section42'].includes(entry.key);
    entry.currentChainPass = isCurrentChainPass(entry, artifactDate, requiresFreshness);
  }
...
function buildLaunchReadiness(entries) {
  ...
  const open = entries.filter(entry => ['stage8', 'stage9', 'stage10', 'stage11', 'section8', 'section42'].includes(entry.key) && !entry.currentChainPass);
...
async function main() {
  ...
  const refreshOpen = entries.filter(entry => ['stage8', 'stage9', 'stage10', 'stage11', 'section8', 'section42'].includes(entry.key) && !entry.currentChainPass);
  const docs = {
  ...
  const payload = {
    ...
    summary: {
      ...
      greenCurrentChainKeys: entries.filter(entry => entry.currentChainPass).map(entry => entry.key)
    },
    entries
  };

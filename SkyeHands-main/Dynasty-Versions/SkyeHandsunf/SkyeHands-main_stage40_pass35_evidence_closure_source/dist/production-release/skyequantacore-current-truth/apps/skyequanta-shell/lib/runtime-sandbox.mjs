...
export function getRuntimeSandboxPolicy(env = process.env) {
  return {
    ...
    memoryMb: Math.max(64, readInteger(env.SKYEQUANTA_RUNTIME_LIMIT_MEMORY_MB, 512)),
    processCount: Math.max(8, readInteger(env.SKYEQUANTA_RUNTIME_LIMIT_NPROC, 128)),
...
function buildInnerShellCommand(command, args, options = {}) {
  ...
  const prlimit = options.support?.prlimit && options.limits?.enabled
    ? `${shellQuote(options.support.prlimit)} ${buildPrlimitArgs(options.limits).map(shellQuote).join(' ')} -- `
    : '';
...
export function buildRuntimeSandboxLaunch(command, args = [], options = {}) {
  ...
  if (effectiveMode === 'off' || effectiveMode === 'process') {
    ...
    const wrappedProcessCommand = processContainment.shellCommand
      ? `${support.prlimit && limits.enabled ? `${shellQuote(support.prlimit)} ${buildPrlimitArgs(limits).map(shellQuote).join(' ')} -- ` : ''}${shellQuote(support.bash || 'bash')} -lc ${shellQuote(processContainment.shellCommand)}`
      : null;
    ...
    if (support.prlimit && limits.enabled) {
      return applyAppArmorLaunch({
        ...
        args: support.prlimit ? [...buildPrlimitArgs(limits), '--', command, ...args] : args,
        cwd: options.cwd,
        scratchDir,
        envAdditions,
        containment: processContainment.metadata,

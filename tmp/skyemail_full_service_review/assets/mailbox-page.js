(async function(){
  ...
  qs('#disconnectBtn').onclick = async ()=> { if(!confirm('Disconnect the Google mailbox from this Skye Mail Vault account?')) return; try{ await SMV.disconnectGoogle(); location.href='/onboarding.html'; }catch(err){ setNote(err.message || 'Disconnect failed.', 'danger'); } };

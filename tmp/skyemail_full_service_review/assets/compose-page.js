(async function(){
  ...
  async function loadSettings(){
    ...
    if(settings?.gmail?.connected) qs('#mailboxBadge').textContent = `Connected mailbox • ${settings.gmail.google_email}`;
    else qs('#mailboxBadge').textContent = 'No Google mailbox connected';
    populateAliases();
  }
  ...
  async function sendMail(){
    try{
      ...
      const data = await apiFetch('/gmail-send', { method:'POST', body: JSON.stringify(payload) });

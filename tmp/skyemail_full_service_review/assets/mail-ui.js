window.SMV = (function(){
  ...
  function renderRail({ activeId, labelMap = {}, status = null }){
    ...
    if(!rail) return;
    const mailbox = status && status.connected && status.mailbox ? status.mailbox.google_email : 'No mailbox connected';
    const watch = status && status.connected && status.mailbox ? (status.mailbox.watch_status || 'inactive') : 'inactive';
    rail.innerHTML = `
      <div class="rail-card">
      ...
      <div class="rail-card">
        ...
        <div class="btnrow" style="margin-top:0">
          ...
          <a class="btn" href="/onboarding.html">Onboarding</a>
      ...
      <div class="topbar">
        <div class="nav">
          <div class="brand">
            ...
            <div class="name"><b>Skye Mail Vault</b><span>${safe(pageName || 'Mail command center')} • ${safe(pageHint || 'Google-backed mailbox control plane')}</span></div>
          </div>
          <div class="navlinks">
  ...
  async function connectGoogle(next='/dashboard.html'){
    const data = await apiFetch(`/google-oauth-start?mode=json&next=${encodeURIComponent(next)}`);

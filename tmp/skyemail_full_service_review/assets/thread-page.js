(async function(){
  ...
  function renderMessage(m){
    return `<article class="thread-message">
      ...
      ${m.attachments?.length ? `<div class="attachments">${m.attachments.map((a)=>`<a class="attachment" href="/.netlify/functions/gmail-attachment?id=${encodeURIComponent(m.id)}&attachmentId=${encodeURIComponent(a.attachment_id)}&filename=${encodeURIComponent(a.filename)}">${safe(a.filename)} • ${safe(a.mime_type)}</a>`).join('')}</div>` : ''}
